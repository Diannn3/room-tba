define(['exports'], (t) => {
			...
			(y = Object.assign
				...
				: function (t) {
						for (var e = 1; e < arguments.length; e++) {
							var s = arguments[e];
							for (var n in s) Object.hasOwn(s, n) && (t[n] = s[n]);
						}
						return t;
					}),
	...
	function x(t) {
		return t !== IDBDatabase.prototype.transaction || 'objectStoreNames' in IDBTransaction.prototype
			? (
				...
				? function (...e) {
						return t.apply(C(this), e), I(R.get(this));
					}
				: function (...e) {
						return I(t.apply(C(this), e));
					}
			: function (e, ...s) {
					const n = t.call(C(this), e, ...s);
	...
	function B(t, e) {
		...
		const s = e.replace(/FromIndex$/, ''),
			n = e !== s,
			i = N.includes(s);
		if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || (!i && !E.includes(s))) return;
	...
	class T {
		...
		async expireEntries(t, e) {
			...
			for (; n; ) {
				const s = n.value;
				s.cacheName === this.u && ((t && s.timestamp < t) || (e && r >= e) ? i.push(n.value) : r++),
					(n = await n.continue());
			}
			const a = [];
			for (const t of i) await s.delete(k, t.id), a.push(t.url);
			return a;
		}
		m(t) {
			return this.u + '|' + M(t);
		...
		async getDb() {
			return (
				this.h ||
					(this.h = await ((t, e, { blocked: s, upgrade: n, blocking: i, terminated: r } = {}) => {
						const a = indexedDB.open(t, e),
							o = I(a);
						return (
							n &&
								a.addEventListener('upgradeneeded', (t) => {
									n(I(a.result), t.oldVersion, t.newVersion, I(a.transaction), t);
								}),
	...
	class P {
		constructor(t = {}) {
			(this.L = t.statuses), (this.I = t.headers);
		}
		isResponseCacheable(t) {
			let e = !0;
			return (
				this.L && (e = this.L.includes(t.status)),
				this.I && e && (e = Object.keys(this.I).some((e) => t.headers.get(e) === this.I[e])),
				e
			);
		}
	}
	...
	class A {
		constructor(t, e) {
			(this.C = {}),
				...
				(this.k = [...t.plugins]),
				(this.M = new Map());
			for (const t of this.k) this.M.set(t, {});
				...
				f = l
					? await (async (t, e, s, n) => {
							...
							const r = Object.assign(Object.assign({}, n), { ignoreSearch: !0 }),
								a = await t.keys(e, r);
							for (const e of a) if (i === W(e.url, s)) return t.match(e, n);
		...
		async getCacheKey(t, e) {
			...
			if (!this.C[s]) {
				...
				for (const t of this.iterateCallbacks('cacheKeyWillBeUsed'))
					n = K(await t({ mode: e, request: n, event: this.event, params: this.params }));
				this.C[s] = n;
			}
			return this.C[s];
		}
		...
		*iterateCallbacks(t) {
		...
		waitUntil(t) {
			return this.B.push(t), t;
		}
		async doneWaiting() {
			for (; this.B.length; ) {
				const t = this.B.splice(0),
	...
	class F {
		...
		handleAll(t) {
			...
			const e = t.event,
				...
				r = this.j(i, s, e);
			return [r, this.P(r, i, s, e)];
		}
		async j(t, e, n) {
			let i;
			await t.runCallbacks('handlerWillStart', { event: n, request: e });
			try {
				if (((i = await this.W(e, t)), !i || 'error' === i.type))
	...
	(t.CacheFirst = class extends F {
		...
		(t.ExpirationPlugin = class {
			...
			Z(t) {
				...
				let e = this.et.get(t);
				return e || ((e = new j(t, this.tt)), this.et.set(t, e)), e;
			}
			Y(t) {
				if (!this.D) return !0;
				const e = this.st(t);
		...
		(t.createHandlerBoundToURL = (t) => Z().createHandlerBoundToURL(t)),
