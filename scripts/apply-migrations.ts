...
function resolveDatabaseUrl(): string {
	...
	const url = (
		prodOnly
			...
			: process.env.DATABASE_URL?.trim() || process.env.PROD_DATABASE_URL?.trim()
	)?.trim();
 ...
 */
async function baselineExistingDatabase(client: pg.Client, files: string[], applied: Set<string>) {
	...
	console.log(`Baselined ${files.length} migrations on existing database (no SQL re-run)`);
}

async function applyPendingMigrations(client: pg.Client, files: string[], drizzleDir: string) {
