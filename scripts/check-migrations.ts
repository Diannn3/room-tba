...
const REQUIRED_COLUMNS = [
	...
	['rooms', 'photos'],
	['dorms', 'photos'],
	['buildings', 'street_view_pano_id'],
	['buildings', 'street_view_captured'],
	['buildings', 'street_view_distance_m'],
	['buildings', 'street_view_checked_at']
] as const;
...
async function main() {
	...
	try {
		...
		if (missing.length > 0) {
			throw new Error(`Missing tables (apply drizzle migrations): ${missing.join(', ')}`);
		}
		...
		for (const [table, column] of REQUIRED_COLUMNS) {
         ...
         WHERE table_schema = $1
           ...
           AND column_name = $3`,
			...
			if (rows.length === 0) missingColumns.push(`${table}.${column}`);
		}
		if (missingColumns.length > 0) {
			throw new Error(`Missing columns (apply drizzle migrations): ${missingColumns.join(', ')}`);
		}
		...
		const missingSyncRows = REQUIRED_SYNC_ROWS.filter((table) => !presentSyncRows.has(table));
