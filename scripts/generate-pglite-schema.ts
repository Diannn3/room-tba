...
function columnDdl(col: LocalColumn, forAlter: boolean): string {
	...
	if (col.notNull && !col.primaryKey && (!forAlter || col.default !== undefined))
		parts.push('NOT NULL');
	return parts.join(' ');
}
...
export function buildPgliteInitSql(): string {
	...
	for (const table of SYNCED_TABLES) {
		...
		for (const col of cols) {
			...
			statements.push(`ALTER TABLE "${name}" ADD COLUMN IF NOT EXISTS ${columnDdl(col, true)};`);
			...
			if (!col.notNull && !col.primaryKey) {
				statements.push(`ALTER TABLE "${name}" ALTER COLUMN "${col.name}" DROP NOT NULL;`);
			}
		}
	}
...
if (import.meta.main) {
	const outPath = new URL('../src/lib/local/data/pglite-schema.generated.ts', import.meta.url);
	const { writeFileSync } = await import('node:fs');
	writeFileSync(outPath, buildGeneratedModule());
