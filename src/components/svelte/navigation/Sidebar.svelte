...
>
  <div class="top">
    <div class="sidebar-header">
      ...
      <!-- Header control, never a labelled row: as an inline NavLink it read
           as a destination named "Collapse menu" sitting above Maps. -->
      <NavLink
        active={false}
        expanded={false}
    ...
    <div class="categories">
      ...
      <NavLink
        ...
        active={sidebarStore.panelOpen === "calendar"}
        expanded={labeled}
  ...
  <div class="bottom">
    <section class="nav-group">
      ...
      {#if contributorsOpen}
        <div
          class="nav-links map-categories"
    ...
    <section class="nav-group">
      ...
      {#if settingsOpen}
        <div
          class="nav-support map-categories"
          transition:slide={{ axis: "y", duration: 300 }}
        >
          <NavLink
            active={false}
          ...
          <NavLink
            ...
            onclick={() => {
              window.dispatchEvent(
                new CustomEvent("room-tba:open-shortcuts-help"),
              );
            }}
...
<style>
  aside {
    ...
    z-index: 200;
    flex: 0 0 auto;
    background-color: hsl(5 20% 97%);
    transition: flex var(--motion-duration-fast, 150ms) ease;
  }
  ...
  /* Collapsed rail is 4rem wide, so the toggle cannot share the logo row
     there; everywhere else it sits in the header, pushed to the outer edge. */
  .sidebar-header {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  /* 44px hit target in every state; border-box keeps it inside the 4rem
     collapsed rail (.nav-link carries 8px transparent side borders). */
  .sidebar-header :global(.nav-link) {
    box-sizing: border-box;
    min-width: 2.75rem;
    min-height: 2.75rem;
    flex: 0 0 auto;
  ...
  aside.sidebar--expanded .sidebar-header {
    flex-direction: row;
    gap: 0.5rem;
  }

  aside.sidebar--expanded .sidebar-header :global(.nav-link) {
    margin-left: auto;
  }
  ...
  @media (max-width: 48rem) {
    ...
    aside .sidebar-home > svg {
      width: 2.25rem;
      height: 2.25rem;
    }

    /* Drawer is 13rem wide: same header row as the expanded rail, where the
       toggle means "close". */
    ...
    aside .sidebar-header :global(.nav-link) {
      margin-left: auto;
    }
