...
export function columnBoundaries(text: string): number[] {
	const header = text
		.split('\n')
		.find((line) => line.includes('FIRST SEMESTER') && line.includes('SECOND SEMESTER'));
	if (!header) throw new Error('Column header row not found');

	const centres = ['FIRST SEMESTER', 'SECOND SEMESTER', 'MIDYEAR'].map((token) =>
		centreOf(header, token)
	);
...
export function sliceTable(text: string): TableRow[] {
	...
	const headerIndex = lines.findIndex((line) => line.includes('FIRST SEMESTER'));
	// Footnotes ("* For the Midyear session…") and the holidays table below are
	// not milestone rows.
	const endIndex = lines.findIndex(
		(line, index) =>
			index > headerIndex && (/^\s*\*+\s/.test(line) || line.includes('OFFICIAL HOLIDAYS'))
	);

	const body = lines.slice(headerIndex + 1, endIndex === -1 ? lines.length : endIndex);

	const sliced = body.map((line) => ({
		indented: /^\s/.test(line.slice(0, labelEnd)) && line.slice(0, labelEnd).trim() !== '',
	...
	const belongsToNextRow = (index: number) => {
		for (let next = index + 1; next < sliced.length; next += 1) {
			...
			return candidate.cells.some((cell) => !NO_DATE_CELL.test(cell) && /^[-–—]/.test(cell));
		}
		return false;
	};
	...
	for (const [index, line] of sliced.entries()) {
		...
		if (!label) {
			if (belongsToNextRow(index)) {
				heldOver = heldOver
					? (heldOver.map((cell, column) => `${cell} ${cells[column] ?? ''}`.trim()) as [
							string,
							string,
							string
						])
			...
			for (const column of [0, 1, 2] as const) {
				if (cells[column]) {
					previous.cells[column] = `${previous.cells[column]} ${cells[column]}`.trim();
				}
			}
			continue;
		}
...
function resolveYear(month: number, column: number, ayStartYear: number) {
	...
	const distance = (year: number) => Math.abs((year - anchorYear) * 12 + month - anchor.month);
	return distance(anchorYear) <= distance(anchorYear - 1) ? anchorYear : anchorYear - 1;
}
...
export function parseCell(
	...
	const dates: string[] = [];
	for (const match of trimmed.matchAll(/(\d{1,2})\s+([A-Za-z]{3,9})\.?,?\s*(\d{4})?/g)) {
		const day = match[1]?.padStart(2, '0');
		const month = MONTHS[(match[2] ?? '').slice(0, 3).toLowerCase()];
		if (!day || !month) continue;
		const explicitYear = match[3] ? Number(match[3]) : null;
		const year = explicitYear ?? resolveYear(Number(month), column, ayStartYear);
...
function kindOf(label: string, window: { startsOn: string; endsOn: string }) {
	...
	return window.startsOn === window.endsOn ? ('milestone' as const) : ('period' as const);
}
...
function assertOrder(termId: number, ordered: (string | null | undefined)[]) {
	...
	for (let index = 1; index < dates.length; index += 1) {
		...
		if (previous && current && previous > current) {
			throw new Error(`Term ${termId}: dates out of order (${previous} then ${current})`);
		}
	}
}
