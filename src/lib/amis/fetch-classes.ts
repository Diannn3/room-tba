...
export function readAmisPageMeta(payload: unknown, page: number): AmisPageMeta {
	...
	const total = readNumber(obj.total) ?? readNumber(asNested(obj, ['classes', 'total']));
	const lastPageRaw =
		readNumber(obj.last_page) ?? readNumber(asNested(obj, ['classes', 'last_page']));
...
async function fetchAmisPage(
	...
	const response = await fetch(buildClassesUrl(options.termId, page, pageSize), { headers });
	...
	if (!response.ok) {
		...
		throw new AmisImportError(code, `AMIS fetch failed (page ${page}): ${message}`);
	}

	return payload;
}

export async function fetchAmisClasses(options: AmisFetchOptions): Promise<AmisClassRow[]> {
