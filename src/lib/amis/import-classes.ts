...
import { isNonRoomClassType, isRoomScheduledClassType } from './room-scheduled-types';
...
export function buildRoomLookup(
	...
	for (const room of rooms) {
		...
		addSquashed(squashFacilityKey(expandFacilityAbbreviations(key)), room.id, room.code);
	}
	...
	const combinedRoomIdByKey = new Map([...directRoomIdByKey, ...aliasRoomIdByKey]);
...
export function matchRoomId(
	...
	for (const candidate of candidates) {
		const expanded = expandFacilityAbbreviations(normalizeFacilityKey(candidate));
...
export function resolveImportRows(
	...
	for (const row of normalized) {
		if (isNonRoomClassType(row.type)) {
			const roomId = applyRoomMatch(row.facilityCode, lookup, stats, unmatched, false);
		...
		const roomId = applyRoomMatch(row.facilityCode, lookup, stats, unmatched, true);
...
export function summarizeImportChanges(input: {
	...
	for (const [key, row] of incomingByKey) {
		...
		const changed =
			...
			(row.acadGroup !== undefined && (existing.acadGroup ?? null) !== row.acadGroup) ||
			(row.acadOrg !== undefined && (existing.acadOrg ?? null) !== row.acadOrg) ||
