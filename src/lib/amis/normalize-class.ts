...
function looksLikeClassRow(value: unknown): value is AmisClassRow {
	...
	return (
		row.section != null || row.course_code != null || row.courseCode != null || row.type != null
	);
}
...
function resolveFacilityCode(row: AmisClassRow): string | null {
	const direct =
		asString(row.facility_id) ?? asString(row.facility_code) ?? asString(row.room_code);
...
export function normalizeFacilityKey(code: string) {
	return code.trim().toUpperCase().replace(/[-.]/g, ' ').replace(/\s+/g, ' ').trim();
}

function resolveCourseTitle(row: AmisClassRow, courseCode: string): string {
	const course = asRecord(row.course);
	const base =
		asString(course?.title) ?? asString(row.course_title) ?? asString(row.title) ?? courseCode;
...
export function normalizeAmisClass(row: AmisClassRow, termId: number): NormalizedAmisClass | null {
