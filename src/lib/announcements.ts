...
export function isPublished(row: AnnouncementWindow, now: Date = new Date()): boolean {
...
export function sortNewestFirst<T extends AnnouncementWindow & { id: number }>(rows: T[]): T[] {
	return [...rows].sort((a, b) => instant(b.startsOn) - instant(a.startsOn) || b.id - a.id);
}
...
export function unreadCount(rows: AnnouncementWindow[], lastSeenAt: number): number {
...
export function relativeDateLabel(value: string, now: Date = new Date()): string {
