...
export function validateContributorSignup(input: SignupInput): SignupValid | SignupInvalid {
	...
	if (password.length < MIN_CONTRIBUTOR_PASSWORD_LENGTH) {
		return fail(`Password must be at least ${MIN_CONTRIBUTOR_PASSWORD_LENGTH} characters.`);
	}
