	...
	>
		...
		<form class="login-body entity-editor-form" onsubmit={submit}>
			{#if !showForgotPassword}
				{#if isSignup}
					<p class="login-lead">
						Create a free contributor account so your proposals are credited to you and your
						username is reserved. Editors and admins sign in here too.
					</p>
				{/if}
				...
				{#if isSignup}
					<EntityEditorFormField label="Confirm password" inputId="admin-signup-confirm">
						{#snippet control()}
							<input
					...
					<EntityEditorFormField label="Email (optional)" inputId="admin-signup-email">
						{#snippet control()}
							<input
			...
			{/if}
			{#if error}
				<EntityEditorMessage variant="error" message={error} id={loginErrorId} />
			{/if}
			{#if !showForgotPassword}
				...
				<p class="login-mode-toggle">
					{#if isSignup}
						...
						<button type="button" class="login-mode-btn" onclick={() => switchMode('signin')}>
							Sign in
						</button>
					{:else}
						No account yet?
						<button type="button" class="login-mode-btn" onclick={() => switchMode('signup')}>
							Sign up
						</button>
					{/if}
				</p>
			...
			{#if showForgotPassword}
				<div class="forgot-password-panel">
					...
					{:else}
						<EntityEditorFormField label="Username or email" inputId="forgot-password-login">
							{#snippet control()}
								<input
