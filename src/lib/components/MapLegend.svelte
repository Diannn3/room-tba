<script lang="ts">
	...
	import { campusTransit } from '$lib/campus.configtype Props = {
		embedded?: boolean;
		trigger?: 'icon' | 'chip';
	};
	...
	onMount(() => registerEphemeralOverlayDismisser(() => floatingControlPanelStore.close(panelId)));
...
<div class="map-legend" class:embedded class:map-legend--open={open}>
	{#if showPanel}
		<div id="map-icon-legend" class="legend-panel" class:embedded aria-label="Map icon legend">
			{#if !embedded}
				...
				<section class="legend-section" aria-labelledby="legend-routes">
					...
					<div class="legend-list">
						{#each routeItems as item (item.key)}
							<div class="legend-item">
								<span class={`legend-swatch ${item.key}`} aria-hidden="true"> 1 </span>
								<span class="legend-copy">
									<span class="legend-label">{item.label}</span>
									<span class="legend-description">{item.description}</span>
								</span>
