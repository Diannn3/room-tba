...
import { measureRouteStore } from '$lib/stores.svelte';
...
describe('MeasureRoutePanel', () => {
	...
	test("mode pills show totals, and 'no route' when a leg has none", () => {
		seedTwoLegRoute();
		render(MeasureRoutePanel);
		expect(screen.getByRole('button', { name: 'Walk' })).toHaveTextContent('6 min');
		// Drive has an unroutable second leg, so the pill degrades.
		expect(screen.getByRole('button', { name: 'Car / e-bike' })).toHaveTextContent('no route');
	});
	...
	test('always credits OpenStreetMap for the path network', () => {
		render(MeasureRoutePanel);
		expect(screen.getByText(/paths © OpenStreetMap contributors/i)).toBeInTheDocument();
	});
});
