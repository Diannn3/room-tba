<script lang="ts">
	...
	async function promptInstall() {
		...
		if (outcome === 'accepted') {
			isInstalled = true;
			toastStore.show('Room TBA installed — open it from your home screen.', 'success');
		}
		deferredPrompt = null;
	}
	...
	const showPrompt = $derived(!isInstalled && !dismissed && deferredPrompt !== null);
</script>
...
{#if showPrompt}
	<div class="pwa-install-prompt" role="status">
		...
		<button class="pwa-install-action" type="button" onclick={promptInstall}> Install </button>
		<IconButton
			size="sm"
