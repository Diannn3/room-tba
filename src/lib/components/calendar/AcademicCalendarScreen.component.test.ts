...
const term = (id: number, overrides: Partial<TermWithCount> = {}): TermWithCount => ({
	id,
	label: `Term ${id}`,
...
describe('AcademicCalendarScreen', () => {
	...
	test('renders a dialog with the year strip, today marker, and in-session highlight', () => {
		...
		expect(screen.getByRole('dialog', { name: 'Academic Calendar' })).toBeTruthy();

		const segments = document.querySelectorAll('.acal-seg');
		expect(segments).toHaveLength(2); // 9003 has no dates anywhere
		expect(document.querySelector('.acal-seg--past')?.textContent).toContain('1st sem');
		expect(document.querySelector('.acal-seg--in-session')?.textContent).toContain('2nd sem');
	...
	test('lists every active term with CRS id, date range, class count, and status', () => {
		...
		const inSession = cards.find((card) => card.textContent?.includes('CRS 9002'));
		...
		const undated = cards.find((card) => card.textContent?.includes('CRS 9003'));
		expect(undated?.textContent).toContain('Dates TBA');
	});

	test('shows the community-data disclaimer', () => {
		render(AcademicCalendarScreen);
		expect(screen.getByRole('note').textContent).toContain('official UPLB academic calendar');
	});
	...
	describe('with the real AY 2026-2027 registrar calendar', () => {
		...
		test('lists the student-planning milestones, not the staff rows', () => {
			render(AcademicCalendarScreen);
			const labels = [...document.querySelectorAll('.acal-milestone__label')].map(
			...
			expect(labels).not.toContain('University Council Executive Committee Meeting');
		});
		...
		test('marks each milestone on the strip without overlap', () => {
			...
			const lefts = dots.map((dot) => Number.parseFloat(dot.style.left)).sort((a, b) => a - b);
		...
		test('shows the date range for a period and a single date for a deadline', () => {
			...
			const rowFor = (label: string) =>
				[...document.querySelectorAll('.acal-milestone')].find((row) =>
					row.querySelector('.acal-milestone__label')?.textContent?.includes(label)
				);

			expect(rowFor('Final examinations')?.textContent).toContain('Dec 1 – Dec 7');
			expect(rowFor('Dropping deadline')?.textContent).toContain('Oct 28');
			expect(rowFor('Dropping deadline')?.classList.contains('acal-milestone--deadline')).toBe(
				true
			);
		});
		...
		test('a term card opens to its own registrar dates', () => {
			...
			const labels = [
				...(card?.querySelectorAll('.acal-card__dates .acal-milestone__label') ?? [])
		...
		test("a term card only lists its own term's dates", () => {
			...
			const datesIn = (card: Element) =>
				[...card.querySelectorAll('.acal-card__dates .acal-milestone')]
					.filter((row) =>
						row.querySelector('.acal-milestone__label')?.textContent?.includes('Start of classes')
					)
					.map((row) => row.querySelector('.acal-milestone__date')?.textContent);
		...
		test('owns up to the registrar dates that fall off the strip', () => {
			...
			const labelsIn = (selector: string) =>
				[...document.querySelectorAll(`${selector} .acal-milestone__label`)].map(
					(node) => node.textContent
				);

			expect(labelsIn('.acal-milestones__list')).not.toContain('General registration');
			expect(labelsIn('.acal-card__dates')).toContain('General registration');

			const note = document.querySelector('.acal-milestones')?.textContent?.replace(/\s+/g, ' ');
