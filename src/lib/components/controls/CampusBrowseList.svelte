				...
				{#each filteredJeepneyRoutes as route (route.id)}
					<li>
						...
						>
							...
							<span class="entity-list-row__meta">{route.stops.length} stops</span>
						</button>
					</li>
				{/each}
			</ul>
		{:else if activeTab !== 'jeepney' && loaded && visibleCount > 0}
			<ul class="entity-nav-list">
				{#each visibleItems as item (item.id)}
					<li>
						<button type="button" class="entity-list-row" title={item.label} onclick={item.open}>
							<span class="entity-list-row__label">{item.label}</span>
							{#if item.meta}
								<span class="entity-list-row__meta">{item.meta}</span>
							{/if}
							<ChevronRight size={16} aria-hidden="true" class="entity-list-row__chevron" />
						</button>
					</li>
				{/each}
			</ul>
				...
				{#snippet icon()}
					<svg viewBox="0 0 180 128" fill="none" aria-hidden="true">
						...
						<rect
							...
							stroke-width="3"
						/>
						<path d="M69 36v66M111 36v66" stroke="currentColor" stroke-width="2" opacity=".25" />
						<path
