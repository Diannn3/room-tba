<script lang="ts">
	...
	import { getDivisionShareUrl } from '$lib/utils/share-links';
	...
	const divisionShareUrl = $derived(division ? getDivisionShareUrl(division.divisionName) : '');
	...
	$effect(() => {
		...
		void (async () => {
			const counts = await fetchRoomClassCounts('division', id, termId ?? undefined);
	...
	async function submitAllChanges() {
		...
		try {
			...
			if (outcome.proposal) {
				...
				toastStore.show('Division change suggestion submitted for review.', 'success');
			}
	...
	function divisionSavedMessage(field: 'divisionName' | 'collegeId' | null): string | null {
		if (!field) return null;
		if (!canPublish) {
			return entityEditorSavedMessage({ canPublish: false });
		}
		return field === 'collegeId' ? 'Parent college saved.' : 'Division name saved.';
	}
</script>
	...
	{#if division}
		<header class="entity-header">
			...
			<div class="entity-actions">
				<EntityShareCopyLink url={divisionShareUrl} entityLabel={division.divisionName} />
				<EntityEditorToggle
					expanded={editing}
					{canPublish}
