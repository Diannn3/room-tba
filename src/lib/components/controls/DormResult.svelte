<script lang="ts">
	...
	$effect(() => {
		...
		capacityDraft =
			current.capacity === null || current.capacity === undefined ? '' : String(current.capacity);
	...
	function enablePinProposal() {
		...
		toastStore.show(`Drag the ${current.dormName} pin on the map, then release to submit.`, 'info');
	}
	...
	function fieldIsUnchanged(field: DormEditableField, current: DormData) {
		switch (field) {
			...
			case 'contactPhone':
				return arraysEqual(linesToList(contactPhoneDraft), current.contactPhone ?? []);
			case 'amenities':
	...
	async function saveField(field: DormEditableField) {
		...
		try {
			...
			if (outcome.proposal) {
				...
				toastStore.show(`Suggestion for ${current.dormName} submitted for review.`, 'success');
			}
			savedField = field;
			setTimeout(() => {
	...
	{#if dorm}
		<header class="entity-header">
			...
			<div class="entity-meta-row">
				...
				{/if}
				{#if genderLabel}
					<span class="entity-meta-chip gender-badge" style:--badge-color={genderColor}>
						<Users size={12} />
						{genderLabel}
					</span>
				{/if}
			...
			<div class="entity-actions">
				{#if dorm.lon && dorm.lat}
					<EntityDirectionsChip lat={dorm.lat} lon={dorm.lon} destinationLabel={dorm.dormName} />
		...
		{#if !editing}
			<div class="entity-body entity-body--compact">
				...
				{/if}
				<EntityLastUpdated updatedAt={dorm.updatedAt} entityType="dorm" entityId={dorm.id} />
			</div>
		{/if}
				...
				{#if contactPhones.length > 0}
					<div class="entity-detail-row">
						...
						<div class="entity-detail-row__content">
							...
							<span class="entity-detail-row__value">{contactPhones.join(', ')}</span>
						</div>
					</div>
				{/if}
