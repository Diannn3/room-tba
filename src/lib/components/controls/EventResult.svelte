...
<div class="event-result">
	{#if event}
		<header class="entity-header event-header">
			...
			<EntityLastUpdated updatedAt={event.updatedAt} entityType="event" entityId={event.id} />
		</header>
		...
		{#if editing}
			...
			>
				<EntityEditorPanel
					...
					historyEntity={event
						? { entityType: 'event', entityId: event.id, version: event.version }
						: null}
				...
				>
					...
					>
						<EntityEditorFormField label="Title" inputId="event-title-editor">
							{#snippet control()}
								<input id="event-title-editor" bind:value={form.title} required />
							{/snippet}
						</EntityEditorFormField>
						<EntityEditorFormField label="Description" inputId="event-description-editor">
							{#snippet control()}
								<textarea id="event-description-editor" bind:value={form.description}></textarea>
							{/snippet}
						</EntityEditorFormField>
						<EntityEditorFormField label="Category" inputId="event-category-editor">
							{#snippet control()}
						...
						<EntityEditorFormField label="Source URL" inputId="event-source-editor">
							{#snippet control()}
						...
						<ImageUpload
							...
							disabled={saving}
						/>
						<EntityEditorFormField label="Recurrence" inputId="event-recurrence-editor">
							{#snippet control()}
								<select id="event-recurrence-editor" bind:value={form.recurrence}>
						...
						<EntityEditorCard title="Primary location anchor">
							<p>
								Choose whether the primary marker follows a building, dorm, or custom map
								coordinates.
							</p>
							<EntityEditorFormField label="Anchor type" inputId="event-anchor-type-editor">
								{#snippet control()}
									<select
							...
							{#if locationForm.anchorType === 'building'}
								<EntityEditorFormField label="Building" inputId="event-building-anchor-editor">
									{#snippet control()}
										<select
							...
							{:else if locationForm.anchorType === 'dorm'}
								<EntityEditorFormField label="Dorm" inputId="event-dorm-anchor-editor">
									{#snippet control()}
										<select
							...
							{/if}
							<EntityEditorFormField label="Location label" inputId="event-location-label-editor">
								{#snippet control()}
									<input id="event-location-label-editor" bind:value={locationForm.label} />
								{/snippet}
							</EntityEditorFormField>
							<div class="entity-editor-form-actions">
								<EntityEditorSubmitButton
									...
									label={canPublish ? 'Save location anchor' : 'Submit location'}
									savingLabel={canPublish ? 'Saving anchor...' : 'Submitting...'}
						...
						<EntityEditorCard title="Event location">
							{#if primaryLocation && primaryLocation.resolvedLat !== null && primaryLocation.resolvedLon !== null}
								<p>
									{#if canPublish}
										Drag the event marker on the map to move it. This edits the primary location
										only.
							...
							{:else}
								<p>
									Place a primary marker at the current map center, then drag it on the map to
									refine the location.
								</p>
							{/if}
							<div class="entity-editor-form-actions">
								{#if canPublish}
									<EntityEditorSubmitButton
										variant="secondary"
										label={mapEditStore.enabled ? 'Map editing on' : 'Enable map editing'}
										onclick={() => {
											if (!mapEditStore.enabled) mapEditStore.toggle();
										}}
									/>
