<script lang="ts">
	...
	const tabEvents = $derived(activeTab === 'upcoming' ? upcomingEvents : pastEvents);
	const pageCount = $derived(Math.max(1, Math.ceil(tabEvents.length / PAGE_SIZE)));
	const currentPage = $derived(Math.min(page, pageCount));
	const pageStart = $derived((currentPage - 1) * PAGE_SIZE);
	const pageEvents = $derived(tabEvents.slice(pageStart, pageStart + PAGE_SIZE));
