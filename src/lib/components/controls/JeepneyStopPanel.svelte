<script lang="ts">
	...
	const stop = $derived(route && stopIndex !== null ? (route.stops[stopIndex] ?? null) : null);
	...
	<div class="entity-detail jeepney-stop-panel">
		<header class="entity-header">
			<div class="entity-panel-header-top">
				<span class="jeepney-stop-panel__route-badge" style:background-color={route.color}>
		...
		{#key stop.id ?? stopIndex}
			<TransitStopEditor routeId={route.id} routeName={route.name} {stop} onRemoved={closeStop} />
		{/key}

		<div class="entity-actions">
			<div class="jeepney-stop-panel__pager" aria-label="Route stops">
				<MapChromeActionChip toolbar disabled={stopIndex <= 0} onclick={openPreviousStop}>
					<ChevronLeft size={14} aria-hidden="true" />
					Previous stop
				</MapChromeActionChip>
				<MapChromeActionChip
