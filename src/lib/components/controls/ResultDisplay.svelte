...
{#snippet roomsEmptyIcon()}
	<svg viewBox="0 0 180 128" fill="none" aria-hidden="true">
		...
		<rect x="52" y="22" width="76" height="90" rx="8" fill="currentColor" opacity=".1" />
		<rect x="52" y="22" width="76" height="90" rx="8" stroke="currentColor" stroke-width="3" />
		<path
			d="M70 112V38h40v74"
			stroke="currentColor"
			stroke-width="3"
		...
		{:else}
			<div class="building-groups">
				{#each buildingGroups as group (group.key)}
					<section class="building-group" aria-label={group.buildingName ?? 'Unassigned rooms'}>
						...
						<div class="room-list room-list--nested">
							{#each group.rooms as room (room.id)}
								<RoomDisplay {room} searchInput="" classCount={classCounts?.get(room.id)} />
							{/each}
						</div>
					</section>
				{/each}
