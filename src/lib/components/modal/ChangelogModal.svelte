<script lang="ts">
  ...
  import { syncToastStore, modalStore } from "$lib/stores.svelte";
			...
			<section class="changelog-modal__entry">
				<h3 class="changelog-modal__entry-version">
					...
					{#if releaseTimestampLabel(entry.version)}
						<span class="changelog-modal__entry-date">{releaseTimestampLabel(entry.version)}</span>
					{:else if entry.date}
	...
	<div class="changelog-modal__actions">
		{#if hasUpdate}
			...
			<button
				...
				disabled={reloading}
			>
				<RefreshCw size={14} class={reloading ? 'loading-icon' : ''} aria-hidden="true" />
