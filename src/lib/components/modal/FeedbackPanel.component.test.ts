...
describe('FeedbackPanel', () => {
	...
	test('keeps the typed message when the send fails', async () => {
		...
		expect(screen.getByLabelText('Your message')).toHaveValue('directions to CAS are wrong');
	...
	test('keeps the typed message when the network throws', async () => {
		...
		expect(screen.getByLabelText('Your message')).toHaveValue('jeepney route missing');
	});
});
