...
import { jeepneyStore } from '$lib/utils/store.svelte';
...
describe('JeepneyRouteModal', () => {
	test('renders the selected route with fare and stops at 320px', () => {
		...
		expect(screen.getByRole('heading', { name: new RegExp(route.name, 'i') })).toBeVisible();
		expect(screen.getByText(`₱${route.fare.regular}`)).toBeVisible();
		expect(screen.getByText(new RegExp(`\\(${route.stops.length}\\)`))).toBeVisible();
	...
	test('clicking a stop selects its route on the map, then the stop', () => {
		...
		const stopIndex = 2;
		screen.getByRole('button', { name: new RegExp(route.stops[stopIndex].name) }).click();
	...
	test('the side panel copy opens the stop without re-selecting the route', () => {
		...
		screen.getByRole('button', { name: new RegExp(route.stops[1].name) }).click();
