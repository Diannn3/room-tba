...
const PRIMARY_FEATURES = ['Search rooms', 'Explore the map', 'Plan classes', 'Events'];
...
describe('landing modal content balance', () => {
	test('header keeps one value proposition', () => {
		render(LandingModal);
		expect(
			screen.getByText('Find rooms, explore the map, and discover campus events at UPLB.')
		).toBeVisible();
	...
	test('only four feature cards stay on phones', () => {
		...
		const phoneCards = cards.filter((card) => !card.classList.contains('feature-card--secondary'));
	...
	test('secondary cards carry the claims dropped from the header', () => {
		render(LandingModal);
		const secondary = [...document.querySelectorAll('.feature-card--secondary')].map(
...
describe('landing modal controls', () => {
	test('tab bar still switches panels', async () => {
		render(LandingModal);
		expect(screen.getByRole('tabpanel')).toHaveAttribute('id', 'landing-panel-welcome');

		await fireEvent.click(screen.getByRole('tab', { name: 'Campus team' }));
		expect(screen.getByRole('tabpanel')).toHaveAttribute('id', 'landing-panel-campus');
	...
	test("Get Started persists the don't-show-again choice", async () => {
		...
		await fireEvent.click(screen.getByRole('checkbox', { name: /Don't show again/i }));
		await fireEvent.click(screen.getByRole('button', { name: 'Get Started' }));
