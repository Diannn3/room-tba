...
describe('modal scroll chrome', () => {
	...
	test('landing modal content uses shared scroll chrome', () => {
		render(LandingModal);
		expect(document.querySelector('.scroll-region')).toHaveClass('map-chrome-scroll');
	});
	...
	test('campus team renders live editor credits with optional profile links', async () => {
		...
		expect(name.closest('a')).toHaveAttribute('href', 'https://example.com/profile');
	...
	test('schedule modal body uses shared scroll chrome', () => {
		...
		expect(document.querySelector('.schedule-modal__body')).toHaveClass('map-chrome-scroll');
	});
});
