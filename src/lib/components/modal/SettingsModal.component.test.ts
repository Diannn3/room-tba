...
const clearButton = () => screen.getByRole('button', { name: 'Clear cached data and reload' });
...
describe('SettingsModal', () => {
	test('renders every section at 320px without horizontal overflow', () => {
		...
		for (const section of ['View', 'Terrain', 'Schedule', 'Feedback', 'Storage']) {
...
describe('SettingsModal storage section (#865)', () => {
	...
	test('the first click only asks; it clears nothing', async () => {
		...
		expect(screen.getByText(/Downloaded offline maps go too/)).toBeInTheDocument();
		expect(screen.getByRole('button', { name: 'Clear and reload' })).toBeInTheDocument();
	});
	...
	test('moves focus to the confirm so keyboard users are not dropped on <body>', async () => {
		...
		expect(confirm.getAttribute('aria-describedby')).toBe('settings-storage-warning');
	});
...
describe('SettingsModal resync campus data', () => {
	...
	test('reports success once the sync store settles', async () => {
		...
		expect(
			await screen.findByText('Campus data is up to date.', {}, { timeout: 3000 })
		).toBeInTheDocument();
	...
	test('leaves the offline map caches and the clear button alone', () => {
		...
		expect(screen.getByText(/downloaded offline maps are kept/i)).toBeInTheDocument();
		expect(clearButton()).toBeInTheDocument();
	});
