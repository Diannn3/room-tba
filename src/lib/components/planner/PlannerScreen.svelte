<script lang="ts">
	...
	const offerings = $derived.by(() => {
		const seen = new Map<string, { courseCode: string; section: string; titles: string[] }>();
	...
	const courseGroups = $derived.by(() => {
		...
		for (const g of byCourse.values()) {
			g.sections.sort((a, b) => (a.type === 'LEC' ? 0 : 1) - (b.type === 'LEC' ? 0 : 1));
		}
		return [...byCourse.values()];
	});

	const unscheduled = $derived((plan?.sections ?? []).filter((s) => !s.stale && isUnscheduled(s)));
	...
	const planFinals = $derived.by(() => {
		...
		return finals.filter(
			(exam) =>
				...
				(!exam.section || sectionsByCourse.get(exam.courseCode)?.has(exam.section))
		);
	});
	...
	function downloadIcs() {
		...
		const ics = buildPlanIcs(plan, term ? { startsOn: term.startsOn, endsOn: term.endsOn } : null);
	...
	const hasSchedule = $derived((plan?.sections ?? []).some((s) => !s.stale && !isUnscheduled(s)));
...
>
	<header class="planner-header">
		...
		{#if plan}
			<button
				...
				title={hasSchedule ? 'Copy a link that reopens this plan' : 'Add a scheduled class first'}
			>
			...
			<button
				...
				title={hasSchedule ? 'Download the timetable as an image' : 'Add a scheduled class first'}
			>
	...
	<div class="planner-tabs" role="tablist" aria-label="Plans">
		{#each plannerStore.plansForTerm as tabPlan (tabPlan.id)}
			<div class="planner-tab-item" class:planner-tab-item--active={tabPlan.id === plan?.id}>
	...
	<p class="planner-save-note" role="note">
		{#if adminAuthStore.isLoggedIn}
			Plans and professor notes save automatically on this device and sync to your account.
		{:else}
			Plans save automatically on this device. Sign in to keep your plans across devices. Use <strong
				>Share</strong
			> to copy a link you can reopen anywhere or send to someone.
		{/if}
	</p>

	<div class="planner-body" class:planner-body--no-side={!plan || plan.sections.length === 0}>
		<PlannerCourseSearch />
				...
				<h2 class="planner-side__heading">
					...
					{#if plannerStore.conflicts.length > 0}
						<span class="planner-conflict-badge" role="status">
							{plannerStore.conflicts.length} conflict{plannerStore.conflicts.length === 1
								? ''
								: 's'}
						</span>
					{:else if plan && plan.sections.length > 0}
						<span class="planner-conflict-badge planner-conflict-badge--clear" role="status">
							All clear
						</span>
					{/if}
				</h2>
				<p class="planner-side__hint" role="note">
					Room TBA cannot show instructor names from AMIS. Write the professor for each section here
					— saved to your account when signed in, never shared.
				</p>
						...
						<li class="planner-offering planner-offering--group">
							<div class="planner-offering__head">
								...
								<button
									...
									onclick={() =>
										plannerStore.removeOffering(group.courseCode, group.sections[0]?.section ?? '')}
								>
									Remove
								</button>
							</div>
