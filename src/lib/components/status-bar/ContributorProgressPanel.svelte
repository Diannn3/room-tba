<script lang="ts">
	...
	async function loadScope(scope: ScopeTab) {
		...
		try {
			if (scope === 'campus') {
				...
				buildingData = await fetchContributorProgress('building', selectedBuildingId);
			} else {
				mineData = await fetchContributorProgress('mine');
			}
		} catch (loadError) {
			error = loadError instanceof Error ? loadError.message : 'Could not load progress.';
		} finally {
			loading = false;
		}
	}
	...
	function fieldRows(counts: Record<RoomFieldCategory, { filled: number; total: number }>) {
...
<section class="contributor-progress" aria-label="Contributor progress">
	<div class="contributor-progress__tabs" role="tablist" aria-label="Progress scope">
			...
			{#each fieldRows(campusData.rooms) as row (row.category)}
				...
				>
					...
					<div class="map-chrome-progress">
						<div class="map-chrome-progress__value" style:width={`${row.percent}%`}></div>
					</div>
				</div>
			{/each}
		</div>
			...
			<div class="contributor-progress__group" aria-label="Buildings with gaps">
				...
				<ul class="contributor-progress__list">
					{#each campusData.topBuildings as building (building.buildingId)}
						<li>
							<span class="contributor-progress__list-title">{building.buildingName}</span>
	...
	{:else if activeScope === 'mine' && mineData}
		...
		{#if mineData.totalEdits === 0}
			<p class="contributor-progress__status">No published edits in history yet.</p>
		{:else}
			<ul class="contributor-progress__list">
				{#each Object.entries(mineData.byEntityType).sort( ([a], [b]) => a.localeCompare(b) ) as [entityType, count] (entityType)}
					<li>
						<span class="contributor-progress__list-title">{formatEntityType(entityType)}</span>
