...
describe('TodayScreen', () => {
	...
	test('points an empty planner at the Planner instead of a blank screen', async () => {
		...
		await fireEvent.click(screen.getByRole('button', { name: 'Open the Planner' }));
	...
	test("lists today's classes in time order with a room link", async () => {
		...
		expect(
			[...today.querySelectorAll('.today-entry__time')].map((el) => el.textContent?.trim())
	...
	test('renders an explicit empty state per day and a distinct Sunday', () => {
		...
		expect(tomorrow.querySelector('.today-day__empty')?.textContent?.trim()).toBe('No classes.');
