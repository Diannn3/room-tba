...
describe('community-links', () => {
	test('volunteer Messenger defaults to contribute short link', () => {
		...
		expect(MESSENGER_CONTRIBUTE_URL).toBe('https://room-tba.uplb.tools/messenger/contribute');
		expect(MESSENGER_MAINTAIN_URL).toBe('https://room-tba.uplb.tools/messenger/maintain');
		expect(MESSENGER_SHORT_CONTRIBUTE_URL).toBe('https://messenger.uplbtools.me/contribute');
	});
