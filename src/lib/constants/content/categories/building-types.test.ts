...
describe('buildingMatchesTypeFilter', () => {
	...
	it('matches admin buildings to the administrative filter', () => {
		expect(buildingMatchesTypeFilter(building(1, 'admin'), 'administrative-building')).toBe(true);
		expect(buildingMatchesTypeFilter(building(2, 'non-admin'), 'administrative-building')).toBe(
			false
		);
	});

	it('matches non-admin buildings to the class filter regardless of classes', () => {
		expect(buildingMatchesTypeFilter(building(2, 'non-admin'), 'class-building')).toBe(true);
	});

	it('surfaces an admin building that hosts classes under BOTH filters', () => {
		const dual = building(1, 'admin'); // id 1 is in withClasses
		expect(buildingMatchesTypeFilter(dual, 'administrative-building', withClasses)).toBe(true);
		expect(buildingMatchesTypeFilter(dual, 'class-building', withClasses)).toBe(true);
	});

	it('keeps an admin building with no classes out of the class filter', () => {
		const adminOnly = building(9, 'admin'); // not in withClasses
		expect(buildingMatchesTypeFilter(adminOnly, 'class-building', withClasses)).toBe(false);
	});
