...
export function buildingMatchesTypeFilter(
	...
	if (filter === 'class-building') {
		return (
			building.buildingType === 'non-admin' || (buildingIdsWithClasses?.has(building.id) ?? false)
		);
	}
	return false;
}

export function dormMatchesTypeFilter(dorm: DormData, filter: BuildingTypeFilter) {
	if (filter === 'all') return true;
...
export function getBuildingTypeFilterOptions(
	...
	const bump = (filter: BuildingTypeFilter) => counts.set(filter, (counts.get(filter) ?? 0) + 1);
