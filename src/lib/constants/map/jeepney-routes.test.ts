...
describe('deriveRouteLineFromStops', () => {
	...
	test('returns a LineString in [lon, lat] order', () => {
		const line = deriveRouteLineFromStops([stop('A', 14.1, 121.2), stop('B', 14.3, 121.4)]);
...
describe('resolveRouteGeometry', () => {
	...
	test('prefers stored geometry and reports its source', () => {
		...
		expect(resolveRouteGeometry(route, { demo: { source: 'routed', geometry } }).source).toBe(
			'routed'
		);
	});
	...
	test('a route-specific caveat overrides the generic one', () => {
		...
		expect(resolveRouteGeometry(route, { demo: { source: 'routed', geometry } }).caveat).toBe(
			ROUTE_GEOMETRY_NOTES.routed
		);
	});
	...
	test('returns no line when a route has too few stops to join', () => {
		expect(resolveRouteGeometry({ id: 'demo', stops: [stop('A', 14.1, 121.2)] }, {})).toEqual({
			source: 'stops-only',
			line: null,
...
describe('jeepney-geometries.json', () => {
	...
	test('the bundled campus routes still ship road-snapped geometry', () => {
		for (const route of JEEPNEY_ROUTES) {
			expect(resolveRouteGeometry(route, stored).source, route.id).not.toBe('stops-only');
		}
	});
});
