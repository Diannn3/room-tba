...
export async function syncCampusDirectoryForOffline(options?: {
	...
	try {
		const [buildingCheck, collegeCheck, divisionCheck, dormCheck, organizationCheck, placeCheck] =
			await Promise.all([
		...
		const [buildingLoad, collegeLoad, divisionLoad, dormLoad, organizationLoad, placeLoad] =
			await Promise.all([
		...
		localStorage.setItem(OFFLINE_DIRECTORY_SYNCED_AT_KEY, new Date().toISOString());
		...
		return {
			ok: false,
			error: error instanceof Error ? error.message : 'Campus directory download failed.'
		};
	}
}
