import { localTableSyncCheck, syncClasses } from './sync';
...
export async function syncClassSchedulesForOffline(options?: {
	...
	try {
		...
		localStorage.setItem(OFFLINE_CLASSES_SYNCED_AT_KEY, new Date().toISOString());
		...
		return {
			ok: false,
			rowCount: 0,
			error: error instanceof Error ? error.message : 'Class schedule download failed.'
		};
	}
}
