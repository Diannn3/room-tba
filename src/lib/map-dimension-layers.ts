...
function enableBuildingFillLayer(map: MapLibreMap): void {
	...
	map.setLayerZoomRange(BUILDING_FILL_LAYER_ID, BUILDING_FILL_MIN_ZOOM, BUILDING_FILL_MAX_ZOOM_2D);
}

function restoreBuildingFillLayerFor3D(map: MapLibreMap): void {
	if (!map.getLayer(BUILDING_FILL_LAYER_ID)) return;
	map.setLayoutProperty(BUILDING_FILL_LAYER_ID, 'visibility', 'visible');
	map.setLayerZoomRange(BUILDING_FILL_LAYER_ID, BUILDING_FILL_MIN_ZOOM, BUILDING_FILL_MAX_ZOOM_3D);
}
...
export function enterFlatMapDimension(map: MapLibreMap, terrainEnabled: boolean): void {
	syncBuildingLayersForDimension(map, true, terrainEnabled);
}

/** Restore tilted 3D building layers after pitch animation completes. */
export function enterTiltedMapDimension(map: MapLibreMap, terrainEnabled: boolean): void {
