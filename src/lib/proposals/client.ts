...
export async function loadOpenPendingProposals(options?: {
	...
	const open = readStoredProposals().filter((ref) => isOpenProposalStatus(ref.status));
...
export function summarizeProposalPatch(
	...
	if (entityType?.startsWith('create_')) {
		...
		if (entityType === 'create_building' && Array.isArray(patch.rooms)) {
			...
			const roomLabels = rooms.map((room) => room.roomCode?.trim()).filter(Boolean);
	...
	if (patch.lat !== undefined && patch.lon !== undefined && Object.keys(patch).length <= 2) {
...
export async function persistEntityChange(input: PersistEntityChangeInput): Promise<{
	ok: boolean;
	error?: string;
	published?: unknown;
	...
	if (!result.ok) {
		return {
			ok: false,
			error: result.error ?? `Proposal for ${input.entityLabel} could not be submitted.`
		};
	}
...
export function adminPatchPath(entityType: ProposalEntityType, entityId: number): string {
	switch (entityType) {
		case 'building':
...
export type MergeEntityType = 'building' | 'college' | 'division' | 'dorm' | 'room';
...
export async function mergeEntityRecord(input: {
	...
	const res = await fetch(`/api/admin/${config.path}/${input.sourceId}/merge`, {
		...
		body: JSON.stringify({
			...
			...(input.entityType === 'room' ? { preferredRoomCode: input.preferredName } : {})
		})
	});
	const data = await res.json().catch(() => ({}));
	if (!res.ok) {
...
export async function submitEntityProposal(input: {
	...
	const proposalId =
		input.proposalId ?? getStoredProposalForEntity(input.entityType, input.entityId)?.id ?? null;
	...
	const proposal = (data as { proposal?: StoredProposalRef & { status: string } }).proposal;
	if (proposal) {
...
export async function withdrawEntityProposal(input: {
	...
	const res = await fetch(`/api/proposals/${input.proposalId}/withdraw`, {
		...
		credentials: 'same-origin',
		headers: { 'content-type': 'application/json' },
		body: JSON.stringify(input.submitterName ? { submitterName: input.submitterName } : {})
	});
	const data = await res.json().catch(() => ({}));
	if (!res.ok) {
...
export function resolveSubmitterName(input: {
	...
	return input.displayName?.trim() || input.username?.trim() || input.draftName?.trim() || '';
}
