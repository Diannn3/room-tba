...
export function validateBundledRooms(rooms: BundledRoomDraft[], maxRooms = 20): void {
	if (rooms.length > maxRooms) {
		throw new ProposalValidationError(`Add at most ${maxRooms} rooms in one building suggestion.`);
	}
	const codes = new Set<string>();
	for (const room of rooms) {
		const key = room.roomCode.toLowerCase();
		if (codes.has(key)) {
			throw new ProposalValidationError(`Duplicate room code in this suggestion: ${room.roomCode}`);
		}
		codes.add(key);
	}
}
...
export function validateCreateProposalPatch(
	...
	switch (entityType) {
		case 'create_building': {
			const name = typeof patch.buildingName === 'string' ? patch.buildingName.trim() : '';
			if (!name) throw new ProposalValidationError('Building name is required.');
			if (typeof patch.lat !== 'number' || typeof patch.lon !== 'number') {
				throw new ProposalValidationError('Pick a map location for the new building.');
			}
		...
		case 'create_event': {
			...
			if (typeof patch.startsAt !== 'string' || typeof patch.endsAt !== 'string') {
				throw new ProposalValidationError('Event start and end are required.');
			}
			return;
		}
		case 'create_dorm': {
			const name = typeof patch.dormName === 'string' ? patch.dormName.trim() : '';
			const gender = typeof patch.gender === 'string' ? patch.gender.trim() : '';
			if (!name) throw new ProposalValidationError('Dorm name is required.');
			if (!gender) throw new ProposalValidationError('Dorm gender is required.');
			return;
		}
		case 'create_place': {
			...
			if (typeof patch.lat !== 'number' || typeof patch.lon !== 'number') {
				throw new ProposalValidationError('Pick a map location for the new place.');
			}
			return;
		}
		case 'create_room': {
			const code = typeof patch.roomCode === 'string' ? patch.roomCode.trim() : '';
			...
			if (!Number.isInteger(buildingId) || buildingId < 1) {
				throw new ProposalValidationError('Pick the building this room belongs to.');
			}
			return;
		}
		case 'create_college': {
			const name = typeof patch.collegeName === 'string' ? patch.collegeName.trim() : '';
			if (!name) throw new ProposalValidationError('College name is required.');
			return;
		}
		case 'create_division': {
			const name = typeof patch.divisionName === 'string' ? patch.divisionName.trim() : '';
			if (!name) throw new ProposalValidationError('Division name is required.');
			return;
		}
		case 'create_organization': {
			const name = typeof patch.name === 'string' ? patch.name.trim() : '';
			if (!name) throw new ProposalValidationError('Organization name is required.');
			const category = typeof patch.category === 'string' ? patch.category.trim() : '';
			if (!ORG_CATEGORIES.includes(category as OrgCategory)) {
				throw new ProposalValidationError('Pick a valid organization category.');
			}
			if (typeof patch.lat !== 'number' || typeof patch.lon !== 'number') {
				throw new ProposalValidationError('Pick a map location for the new organization.');
			}
			return;
		}
		case 'create_jeepney_stop': {
			const routeId = typeof patch.routeId === 'string' ? patch.routeId.trim() : '';
			const name = typeof patch.name === 'string' ? patch.name.trim() : '';
			const description = typeof patch.description === 'string' ? patch.description.trim() : '';
			if (!routeId || !name || !description) {
				throw new ProposalValidationError('Route, stop name, and description are required.');
			}
			if (
