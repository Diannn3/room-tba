...
describe('detectImageContentType', () => {
	test('detects jpeg, png, and webp signatures', () => {
		...
		expect(
			detectImageContentType(new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00]))
		).toBe('image/png');
		expect(
			detectImageContentType(
...
describe('buildUploadKey', () => {
	test('uses sanitized prefix and extension', () => {
		const key = buildUploadKey('events/my-event', 'image/png', 'test-1111-2222-3333-4444');
...
describe('parseEventImageUrl', () => {
	...
	test('requires https and upload storage origin', () => {
		...
		expect(parseEventImageUrl('https://cdn.example.com/events/a.jpg')).toEqual({
			ok: false,
			error: 'Event image URL requires upload storage (R2_PUBLIC_URL) to be configured'
		});
		expect(
			parseEventImageUrl('https://cdn.example.com/events/a.jpg', 'https://cdn.example.com')
		).toEqual({
			ok: true,
			imageUrl: 'https://cdn.example.com/events/a.jpg',
			provided: true
		});
		expect(parseEventImageUrl('https://evil.example/photo.jpg', 'https://cdn.example.com')).toEqual(
			{
				ok: false,
...
describe('parseImageUrl (generic entity images, #191)', () => {
	test('uses the given label in error messages', () => {
		expect(
			parseImageUrl('http://cdn.example.com/a.jpg', 'https://cdn.example.com', 'Building image')
		).toEqual({
			ok: false,
			error: 'Building image URL must use HTTPS'
		});
		expect(parseImageUrl('https://cdn.example.com/a.jpg', null, 'Room image')).toEqual({
			ok: false,
			error: 'Room image URL requires upload storage (R2_PUBLIC_URL) to be configured'
		});
	});

	test('accepts a valid URL from the configured storage origin', () => {
		expect(
			parseImageUrl('https://cdn.example.com/dorms/a.jpg', 'https://cdn.example.com', 'Dorm image')
		).toEqual({
			ok: true,
	...
	test('rejects a URL from a different origin', () => {
		expect(
			parseImageUrl('https://evil.example/photo.jpg', 'https://cdn.example.com', 'Building image')
		).toEqual({
			ok: false,
