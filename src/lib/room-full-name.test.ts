...
describe('isRoomCodeExpansion', () => {
	test('accepts an acronym token spelled out', () => {
		expect(isRoomCodeExpansion('DSDS MLH', 'DSDS Main Lecture Hall')).toBe(true);
	});
	...
	test('accepts a truncated word plus an acronym, with extra leading words', () => {
		...
		expect(isRoomCodeExpansion('PHYSIO LR', 'Animal Physiology Lecture Room')).toBe(true);
	});
	...
	test('rejects out-of-order words', () => {
		expect(isRoomCodeExpansion('LR PHYSIO', 'Physiology Lecture Room')).toBe(false);
	});
...
describe('pickRoomFullName', () => {
	...
	test('prefers the longest expansion and keeps its original casing', () => {
		expect(
			pickRoomFullName('DSDS MLH', ['DSDS-MLH', 'DSDS Main Lecture Hall', 'DSDS Main Lec Hall'])
