...
export class ScheduleRenderer {
	...
	init() {
		...
		const cssWidth = Math.min(this.config.width, this.canvas.clientWidth || this.config.width);
		const cssHeight = Math.min(this.config.height, this.canvas.clientHeight || this.config.height);
		...
		this.config.colors.background =
			root.getPropertyValue('--map-chrome-surface').trim() || this.config.colors.background;
		this.config.colors.header =
			root.getPropertyValue('--map-chrome-border-accent').trim() || this.config.colors.header;
...
export function parseScheduleTime(scheduleStr: string) {
	...
	const startStr =
		formattedStartHour + (startMin > 0 ? `:${String(startMin).padStart(2, '0')}` : '');
	const endStr = formattedEndHour + (endMin > 0 ? `:${String(endMin).padStart(2, '0')}` : '');
