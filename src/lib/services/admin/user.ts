...
export async function countAdminUsers(): Promise<number> {
	const rows = await db.select({ c: sql<number>`count(*)` }).from(adminUsersTable);
...
export async function getAdminUserBySupabaseId(
	...
	try {
		const [user] = await db
			...
			.where(
				and(eq(adminUsersTable.supabaseUserId, supabaseUserId), eq(adminUsersTable.isActive, true))
			)
			.limit(1);
...
export async function linkOrCreateContributorFromSupabase(
	...
	const email = identity.emailConfirmed ? (identity.email?.trim().toLowerCase() ?? null) : null;
...
async function findUserByLogin(login: string) {
	...
	const [byUsername] = await db
		...
		.from(adminUsersTable)
		.where(and(eq(adminUsersTable.isActive, true), eq(adminUsersTable.username, normalized)))
		.limit(1);
	...
	try {
		const [byEmail] = await db
			.select(adminUserCredentialColumns)
			.from(adminUsersTable)
			.where(and(eq(adminUsersTable.isActive, true), sql`lower(email) = ${normalized}`))
			.limit(1);
		return byEmail ?? null;
	} catch {
...
export async function getAccountProfile(userId: number): Promise<AccountProfile | null> {
	const [row] = await db
		...
		.from(adminUsersTable)
		.where(and(eq(adminUsersTable.id, userId), eq(adminUsersTable.isActive, true)))
		.limit(1);
	if (!row) return null;
	return {
		id: row.id,
...
export async function updateAccountProfile(
	...
	if (!displayName) throw new AccountActionError('Display name cannot be empty.');
	if (
	...
	if (typeof input.showInCredits !== 'undefined' && typeof input.showInCredits !== 'boolean') {
	...
	await db
		...
		.set({
			displayName,
			avatarUrl: optionalHttpsUrl(input.avatarUrl ?? existing.avatarUrl, 'Avatar URL'),
			profileUrl: optionalHttpsUrl(input.profileUrl ?? existing.profileUrl, 'Profile URL'),
...
export async function changePassword(
	...
	if (newPassword.length < MIN_PASSWORD_LENGTH) {
		throw new AccountActionError(`Password must be at least ${MIN_PASSWORD_LENGTH} characters.`);
	}
...
export async function requestEmailChange(userId: number, newEmail: string): Promise<void> {
	...
	const [existing] = await db
		...
		.from(adminUsersTable)
		.where(and(sql`lower(email) = ${normalized}`, ne(adminUsersTable.id, userId)))
		.limit(1);
	if (existing) {
...
export async function linkGoogleIdentity(userId: number, supabaseUserId: string): Promise<void> {
	const [existing] = await db
		.select({ id: adminUsersTable.id })
		.from(adminUsersTable)
		.where(and(eq(adminUsersTable.supabaseUserId, supabaseUserId), ne(adminUsersTable.id, userId)))
		.limit(1);
	if (existing) {
...
export async function exportAccountData(userId: number): Promise<AccountDataExport | null> {
...
export async function createAdminUser(input: CreateAdminUserInput): Promise<AdminManagedUser> {
	...
	try {
		...
		const code =
			...
			((error as { cause?: { code?: string } })?.cause?.code as string | undefined);
		if (code === '23505') {
			throw new AccountActionError('That username or email is already taken.', 409);
		}
		throw error;
	}
}
...
async function countActiveAdmins(tx: Tx | typeof db, excludingUserId?: number): Promise<number> {
	const rows = await tx
		...
		.where(
			and(
				...
				eq(adminUsersTable.isActive, true),
				excludingUserId !== undefined ? ne(adminUsersTable.id, excludingUserId) : sql`true`
			)
		);
	return Number(rows[0]?.c ?? 0);
}
...
export async function updateManagedUser(
	...
	return db.transaction(async (tx) => {
		await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext('admin_users:last-admin-guard'))`);
		...
		if (demotingFromAdmin || deactivatingAdmin) {
			...
			if (remaining < 1) {
				throw new AccountActionError('Cannot remove the last remaining admin.', 400);
			}
		}
