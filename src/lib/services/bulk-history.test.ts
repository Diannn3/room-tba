...
import { buildBackfillOperations, loadSnapshots } from '../../../scripts/backfill-bulk-history';
import {
	BULK_ACTOR,
	bulkRowKey,
	bulkSummary,
...
function fakeDb(seen: { entityType: string; entityId: number; summary: string }[]) {
...
describe('toHistoryRow', () => {
	it('stamps the op key into the summary and defaults action + actor', () => {
		...
		expect(row.summary).toBe('[bulk:2026-08-03-test-op] moved to the right building');
	...
	it('rejects input that would violate the editor_history columns', () => {
		...
		expect(() => toHistoryRow({ ...op, entityType: 'x'.repeat(33) })).toThrow(/entityType/);
		...
		expect(() => toHistoryRow({ ...op, action: 'a'.repeat(33) })).toThrow(/action/);
...
describe('bulkRowKey', () => {
	...
	it('does not match non-bulk history rows', () => {
		expect(bulkRowKey({ entityType: 'room', entityId: 7, summary: 'edited pin' })).toBe('room:7:');
	});
});
...
describe('buildBackfillOperations', () => {
	...
	it('produces rows the recorder accepts', () => {
		for (const entry of operations) expect(() => toHistoryRow(entry)).not.toThrow();
	});
	...
	it('records the merge counts that the snapshot cannot recover', () => {
		const merge = operations.find((entry) => entry.opKey === '2026-08-03-duplicate-room-merge');
	...
	it('emits one row per room when the building snapshot is available', () => {
		...
		const perRoom = withSnapshot.filter(
			(entry) => entry.opKey === '2026-08-03-room-building-audit' && entry.entityId > 0
		);
