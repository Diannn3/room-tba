...
export async function recordProposalContribution(proposal: EditProposalSummary): Promise<void> {
...
export async function getMyContributions(userId: number, limit = 50): Promise<MyContribution[]> {
	return db
		.select({
