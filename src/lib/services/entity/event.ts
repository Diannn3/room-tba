...
export async function getAllEvents(options: EventLoadOptions = {}): Promise<EventData[]> {
	try {
		return hydrateEvents(await loadEventRows(options.includeInactive), options.now);
	} catch (error) {
...
export async function getActiveEvents(now = new Date()): Promise<EventData[]> {
	return (await getAllEvents({ now })).filter((event) => event.status === 'active');
}

export async function getUpcomingEvents(now = new Date()): Promise<EventData[]> {
	return (await getAllEvents({ now })).filter((event) => event.status === 'upcoming');
}
...
async function hydrateEvents(events: EventRow[], now = new Date()): Promise<EventData[]> {
	...
	return events
		.map((event) => {
			...
			const eventLocations = (locationsByEvent.get(event.id) ?? []).map((location) =>
