...
export async function getEntityHistory(
	...
	return db
		...
		.where(
			and(eq(editorHistoryTable.entityType, entityType), eq(editorHistoryTable.entityId, entityId))
		)
...
const REVERTABLE_FIELDS: Record<string, string[]> = {
	building: ['buildingName', 'lat', 'lon', 'buildingType', 'directions', 'imageUrl', 'photos'],
	room: ['roomCode', 'directions', 'buildingId', 'collegeId', 'divisionId', 'imageUrl', 'photos'],
	college: ['collegeName'],
...
function pickRevertInput(
	...
	if (!fields) {
		throw new HistoryRevertError(`History restore is not supported for ${entityType} entries.`);
	}
