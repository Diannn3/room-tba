...
export async function resolveBuildingNameFromSlug(slug: string): Promise<string | null> {
	...
	return rows.find((row) => getBuildingSlug(row) === slug)?.buildingName ?? null;
}

export async function resolveCollegeNameFromSlug(slug: string): Promise<string | null> {
	const rows = await db.select({ collegeName: collegesTable.collegeName }).from(collegesTable);
	return rows.find((row) => getCollegeSlug(row) === slug)?.collegeName ?? null;
}

export async function resolveDivisionNameFromSlug(slug: string): Promise<string | null> {
	const rows = await db.select({ divisionName: divisionsTable.divisionName }).from(divisionsTable);
	return rows.find((row) => getDivisionSlug(row) === slug)?.divisionName ?? null;
}
...
export async function getBuildingPageData(buildingName: string) {
	...
	if (defaultTerm) {
		const [{ count: classTotal }] = await db
			...
			.where(and(eq(roomsTable.buildingId, building.id), eq(classesTable.termId, defaultTerm.id)));
...
export async function getDormPageData(dormId: number) {
	const data = await db.select().from(dormsTable).where(eq(dormsTable.id, dormId));
...
export async function getOrganizationPageData(organizationId: number) {
	const [row] = await db
		...
		.leftJoin(buildingsTable, eq(organizationsTable.buildingId, buildingsTable.id))
...
export async function getPlacePageData(placeId: number) {
	const [place] = await db.select().from(placesTable).where(eq(placesTable.id, placeId)).limit(1);
	return place ?? null;
}
...
export async function getRoomPageData(roomId: number) {
	...
	const [{ count: classCount }] = await db
		...
		.where(
			defaultTerm
				? and(eq(classesTable.roomId, roomId), eq(classesTable.termId, defaultTerm.id))
