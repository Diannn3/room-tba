...
export function getCachedClassCountKey(buildingId: number, termId: number): string {
	return `${buildingId}:${termId}`;
}

export function getCachedRoomClassCountKey(roomId: number, termId: number): string {
