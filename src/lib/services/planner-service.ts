...
export async function savePlannerData(userId: number, data: unknown): Promise<void> {
	const updatedAt = new Date().toISOString();
	await db.insert(plannerPlansTable).values({ userId, data, updatedAt }).onConflictDoUpdate({
