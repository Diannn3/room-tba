...
async function courseHistoryByCode(codes: string[]) {
	...
	const rows = await db
		...
		.where(and(inArray(classesTable.courseCode, codes), isNotNull(classesTable.termId)))
		.groupBy(
...
async function deptHistoryByOrg(orgCodes: string[]) {
	...
	const rows = await db
		...
		.innerJoin(roomsTable, eq(roomsTable.id, classesTable.roomId))
		.innerJoin(buildingsTable, eq(buildingsTable.id, roomsTable.buildingId))
		.where(and(inArray(classesTable.acadOrg, orgCodes), isNotNull(classesTable.termId)))
		.groupBy(
 ...
 */
export async function attachProbableLocations(rows: ClassMapValue[]): Promise<void> {
	const candidates = rows.filter(
		(row) => row.roomId == null && row.roomCode == null && isRoomScheduledClassType(row.type)
	);
	...
	try {
		...
		const courseHistory = await courseHistoryByCode([
			...new Set(unresolved.map((row) => row.courseCode).filter((code): code is string => !!code))
		]);
		...
		for (const row of unresolved) {
			...
			const location = resolveProbableLocation({
				...
				acadGroup: acad?.acadGroup ?? null,
				orgs,
				courseHistory: row.courseCode ? (courseHistory.get(row.courseCode) ?? []) : [],
