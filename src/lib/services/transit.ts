...
export async function createJeepneyStop(input: JeepneyStopCreateInput, editedBy = 'admin') {
	const [route] = await db
		.select({ id: jeepneyRoutesTable.id })
		.from(jeepneyRoutesTable)
		.where(and(eq(jeepneyRoutesTable.id, input.routeId), eq(jeepneyRoutesTable.isActive, true)))
		.limit(1);
...
export async function updateJeepneyStop(
	...
	const where =
		expectedVersion === undefined
			...
			: and(eq(jeepneyStopsTable.id, id), eq(jeepneyStopsTable.version, expectedVersion));
