...
export class PlannerStore {
	...
	plansForTerm = $derived(this.plans.filter((plan) => plan.termId === this.activeTermId));
	...
	activePlan = $derived.by(() => {
		...
		return this.plansForTerm.find((plan) => plan.id === activeId) ?? this.plansForTerm[0] ?? null;
	});
	...
	addOffering = (rows: ClassMapValue[]) => {
		const sections = rows.map(rowToPlannedSection).filter((s): s is PlannedSection => s !== null);
	...
	removeSections = (rows: ClassMapValue[]) => {
		...
		plan.sections = plan.sections.filter((section) => !removeKeys.has(sectionNaturalKey(section)));
		this.persist();
	};
	...
	deletePlan = (id: string) => {
		...
		if (deleted != null && !this.plans.some((plan) => plan.termId === deleted.termId)) {
	...
	private uniqueLabel(base: string, termId: number, exceptId?: string): string {
		const used = new Set(
			this.plans.filter((p) => p.termId === termId && p.id !== exceptId).map((p) => p.label)
		);
