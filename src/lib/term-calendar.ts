...
for (const calendar of [academicCalendar2024, academicCalendar2026]) {
	for (const [termId, window] of Object.entries(calendar.terms)) {
		...
		CHANGE_OF_MATRICULATION_ENDS[Number(termId)] = window.changeOfMatriculationEndsOn;
	}
}

/** Human date ("August 7, 2026") for a term's last day of change of
 *  matriculation, or null when unknown. */
export function changeOfMatriculationLabel(termId: number | null | undefined): string | null {
...
export function isDateWithinTerm(term: Pick<Term, 'startsOn' | 'endsOn'>, date: Date) {
...
export function isFinalsWeek(date: Date, term: Pick<Term, 'id' | 'startsOn' | 'endsOn'>) {
...
export function resolveActiveTermByDate<T extends Term>(terms: T[], date = new Date()): T | null {
...
export function resolveInitialTermId(
	...
	const urlValid = options.fromUrl !== null && terms.some((term) => term.id === options.fromUrl);
	if (urlValid) return options.fromUrl;

	const calendarDefault = resolveActiveTermByDate(terms, options.date);
	const storedTerm =
		options.storedId !== null ? terms.find((term) => term.id === options.storedId) : undefined;
