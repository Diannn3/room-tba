...
const rowFor = (label: string) => sliceTable(FIXTURE).find((row) => row.label.includes(label));
...
describe('sliceTable', () => {
	it('keeps a wrapped cell with the row whose label sits below it', () => {
		...
		expect(rowFor('Transfer from other UP units')?.cells[1]).toBe('09 Dec 2026, Wed');
		expect(rowFor('Completion/Removal')?.cells[1]).toBe('16 Dec 2026, —18 Dec 2026, WedFri');
	});

	it('keeps a range continuation with the row above it', () => {
		expect(rowFor('New First Year Students')?.cells[1]).toBe('4 Nov 2026, Wed -5 Nov 2026, Thu');
		expect(rowFor('New Graduate Students')?.cells[1]).toBe('09 Dec 2026, Wed -11 Dec 2026, Fri');
	});
...
describe('termIdFor', () => {
	it('maps AY start year and column to the CRS id', () => {
		expect([0, 1, 2].map((column) => termIdFor(2026, column))).toEqual([1261, 1262, 1263]);
	});
});
...
describe('parseHolidays', () => {
	it('reads both year columns and skips undated entries', () => {
		...
		expect(holidays.some((holiday) => holiday.label.includes('Eidul'))).toBe(false);
	});
});
...
describe('parseAcademicCalendar', () => {
	...
	it('tags deadlines, periods and single-day milestones', () => {
		const byLabel = (needle: string) =>
			calendar.milestones.find(
				(milestone) => milestone.termId === 1261 && milestone.label.includes(needle)
			);
	...
	it('emits no milestone for a cell the registrar left empty', () => {
		...
		expect(midyearLabels.some((label) => label.includes('LOYALTY'))).toBe(false);
		expect(
			calendar.milestones.some((milestone) => milestone.label.includes('Board of Regents'))
		).toBe(false);
	});
