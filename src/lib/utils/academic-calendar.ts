...
export function buildYearTimeline<T extends DatableTerm>(
	...
	const dated = terms
		...
		.filter((entry): entry is { term: T; window: TermWindow } => Boolean(entry.window))
	...
	const pctAt = (isoDate: string) => ((dayNumber(isoDate) - startDay) / totalDays) * 100;
	...
	const segments = dated.map(({ term, window }) => ({
		...
		widthPct: ((dayNumber(window.endsOn) - dayNumber(window.startsOn) + 1) / totalDays) * 100
	}));

	const months: YearTimeline['months'] = [];
	for (let cursor = rangeStart; cursor <= rangeEnd; cursor = nextMonthStart(cursor)) {
...
/**
 * Grid step for strip markers, in strip percent. The narrowest strip we
 * support is ~296px (320px viewport), where 5% is ~15px — wider than the 12px
 * marker, so two markers can never touch.
 *
 * ponytail: fixed grid, no layout measuring. Entries inside the same cell
...
/** Manila day keys (YYYY-MM-DD) for one dated entry. */
...
export type EventTimeline<E> = {
	...
	/** Entries whose dates miss the strip; surfaced, not silently dropped. */
...
/**
 * Place dated entries on the year strip built by `buildYearTimeline`, and
...
export function buildEventTimeline<E extends EventDays>(
	...
	// Overlap, not containment: an entry that started before the strip but runs
	// into it still belongs to this year — pin it to the first day shown.
	const placed = events
		.filter((event) => event.startsOn <= strip.rangeEnd && event.endsOn >= strip.rangeStart)
		.map((event) => ({
			event,
...
type CalendarTerm = DatableTerm & Pick<Term, 'schoolYear' | 'isActive' | 'sortOrder'>;
...
export function currentAcademicYearTerms<T extends CalendarTerm>(
	...
	const inSession = dated
		...
		.sort((a, b) => b.term.sortOrder - a.term.sortOrder || b.term.id - a.term.id)[0];
	...
	const latest = [...dated].sort((a, b) => b.window.endsOn.localeCompare(a.window.endsOn))[0];

	const anchor = inSession ?? upcoming ?? latest;
	const ay = anchor.term.schoolYear;
	if (!ay) return [anchor.term];
	return dated.filter((entry) => entry.term.schoolYear === ay).map((entry) => entry.term);
}
