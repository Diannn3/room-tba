...
describe('publish window', () => {
	...
	test('expired rows are not published', () => {
		expect(isPublished(row(1, '2026-07-01 08:00:00', '2026-07-25 08:00:00'), NOW)).toBe(false);
	});
	...
	test('open window still running is published', () => {
		expect(isPublished(row(1, '2026-07-25 08:00:00', '2026-07-30 08:00:00'), NOW)).toBe(true);
	});

	test('unparseable end date does not hide the notice', () => {
		expect(isPublished(row(1, '2026-07-20 08:00:00', 'not a date'), NOW)).toBe(true);
	});
