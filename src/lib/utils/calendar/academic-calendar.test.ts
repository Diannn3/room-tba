...
describe('resolveTermWindow', () => {
	it('prefers DB starts_on/ends_on over the hand-kept constants', () => {
		const window = resolveTermWindow(term(1252, { startsOn: '2026-01-05', endsOn: '2026-05-20' }));
...
describe('termWindowStatus', () => {
	...
	it('is inclusive on both edges (Asia/Manila)', () => {
		expect(termWindowStatus(window, manilaNoon('2026-01-19'))).toBe('in-session');
		expect(termWindowStatus(window, manilaNoon('2026-05-31'))).toBe('in-session');
	...
	it('uses the Manila calendar day, not the local/UTC one', () => {
		...
		expect(termWindowStatus(window, new Date('2026-01-18T23:00:00Z'))).toBe('in-session');
	});
});
...
describe('buildYearTimeline', () => {
	...
	it('marks segment status and centers the today marker on its day', () => {
		const timeline = buildYearTimeline(terms, manilaNoon('2026-02-01'));
		expect(timeline?.segments.map((segment) => segment.status)).toEqual(['past', 'in-session']);
	...
	it('returns null when no term has a resolvable window', () => {
		expect(buildYearTimeline([term(9999)], manilaNoon('2026-02-01'))).toBeNull();
	});
});
...
describe('buildEventTimeline', () => {
	...
	it('clusters same-cell entries into one marker with a count', () => {
		...
		expect(markers[0]?.events.map((event) => event.label)).toEqual(['first', 'second', 'third']);
	});

	it('keeps every marker a full grid step apart, even at 60 entries', () => {
		const dense = Array.from({ length: 60 }, (_, index) =>
			entry(`e${index}`, `2026-09-${String((index % 30) + 1).padStart(2, '0')}`)
		);
		...
		for (const [index, marker] of markers.entries()) {
			...
			expect(marker.leftPct).toBeLessThanOrEqual(100 - EVENT_MARKER_STEP_PCT / 2);
			const previous = markers[index - 1];
			if (previous) {
				expect(marker.leftPct - previous.leftPct).toBeGreaterThanOrEqual(EVENT_MARKER_STEP_PCT);
			}
		}
		expect(dense.every((item) => markers.some((marker) => marker.events.includes(item)))).toBe(
			true
		);
	});
	...
	it('groups the list by month in calendar order', () => {
		...
		expect(months[0]?.events.map((event) => event.label)).toEqual(['aug-early', 'aug-late']);
	});
