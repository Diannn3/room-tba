...
export type CampusGateCoordinatesHeader = (typeof CAMPUS_GATE_COORDINATES_HEADERS)[number];

export type GateType = 'vehicle' | 'pedestrian' | 'both';

export type CampusGateCoordinatesRow = Record<CampusGateCoordinatesHeader, string>;
...
export function parseCampusGateCoordinatesCsv(text: string): CampusGateCoordinatesParseResult {
	const lines = text
		.replace(/^\uFEFF/, '')
		.split(/\r?\n/)
