...
function directoryName(source: CampusWebsite) {
	...
	return normalized(title).includes(normalized(short)) ? title : `${title} (${short})`;
}
...
function directoryCategory(
	...
	if (/^(Department|Institute|College|School|General Education|National Service)/i.test(name)) {
		return 'academic';
	}
	if (/Library|Health|Housing|Security|Guidance|Scholar|Student Learning|Teaching/i.test(name)) {
		return 'service';
	}
	return 'unit';
}
