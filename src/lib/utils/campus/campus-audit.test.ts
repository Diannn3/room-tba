...
import {
	...
	spreadMeters,
	tokenize
} from '$lib/utils/campus/campus-audit';
...
describe('parseLocatedAt', () => {
	...
	test('keeps a floor-prefixed host', () => {
		expect(parseLocatedAt('Located at: 3/F Bienvenido M. Gonzalez Hall, main campus.')).toBe(
			'3/F Bienvenido M. Gonzalez Hall'
		);
	});

	test("reads 'Temporarily located at' the same way", () => {
		expect(parseLocatedAt('Temporarily located at: Graduate School (GS) Dormitory')).toBe(
			'Graduate School (GS) Dormitory'
		);
	});

	test('rejects proximity claims, which are not tenancy', () => {
		expect(parseLocatedAt('Located at: Near CAFS complex')).toBeNull();
		expect(parseLocatedAt('Located at: Between Freedom Park and the Math Building.')).toBeNull();
	});

	test('rejects region references that name no host', () => {
		expect(parseLocatedAt('Located at: Forestry (upper) campus')).toBeNull();
		expect(parseLocatedAt('A hall located in the lower campus area (Batong Malake).')).toBeNull();
	});

	test('handles descriptions with no location claim', () => {
		expect(parseLocatedAt('Student council of the College of Arts.')).toBeNull();
...
describe('resolveReference', () => {
	...
	test('matches an acronym only when it is written in caps', () => {
		expect(resolveReference('beside IRNR', index).map((m) => m.entity.id)).toEqual([irnr.id]);
	...
	test('returns every tie rather than silently picking one', () => {
		...
		expect(matches.map((m) => m.entity.id).sort()).toEqual([casAnnex1.id, casAnnex2.id]);
	});
...
describe('geometry', () => {
	...
	test('nearestOf picks the closest candidate', () => {
		...
		expect(nearest?.meters).toBeCloseTo(distanceMeters(gradSchool, internationalHouse), 6);
	});
	...
	test('clusterByProximity groups only what is actually close', () => {
		const clusters = clusterByProximity([gonzalezHall, ovca, casAnnex1, gradSchool], 3);
		expect(clusters).toHaveLength(1);
		expect(clusters[0].map((e) => e.id).sort()).toEqual([gonzalezHall.id, ovca.id].sort());
	});
