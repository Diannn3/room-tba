...
import { groupClassesByOffering, parentLectureSection } from './class-offering-groups.js';
...
describe('parentLectureSection', () => {
	test('maps lab section names to their parent lecture section', () => {
		expect(parentLectureSection(row({ section: 'G-1L', type: 'LAB' }))).toBe('G');
		expect(parentLectureSection(row({ section: 'ST11L', type: 'LAB' }))).toBe('ST1');
		expect(parentLectureSection(row({ section: 'A2-7L', type: 'LAB' }))).toBe('A2');
	});

	test('maps recit section names to their parent lecture (any type)', () => {
		expect(parentLectureSection(row({ section: 'UV-1R', type: 'REC' }))).toBe('UV');
		expect(parentLectureSection(row({ section: 'G-2R', type: 'RECIT' }))).toBe('G');
		expect(parentLectureSection(row({ section: 'AB1R', type: 'RCT' }))).toBe('AB1');
		expect(parentLectureSection(row({ section: 'ST2R', type: 'RCT' }))).toBe('ST2');
	});

	test('returns null for standalone lecture/seminar sections', () => {
		expect(parentLectureSection(row({ section: 'G', type: 'LEC' }))).toBeNull();
		expect(parentLectureSection(row({ section: 'UV', type: 'LEC' }))).toBeNull();
	});
});
...
describe('groupClassesByOffering', () => {
	test('adds the parent lecture row to each lab offering', () => {
		...
		expect(groups[0]?.sections.map((section) => section.section)).toEqual(['G', 'G-1L']);
		expect(groups[1]?.sections.map((section) => section.section)).toEqual(['G', 'G-5L']);
	});
	...
	test('links AGRI 61 compact recits to their lecture section', () => {
		...
		expect(groups.map((group) => group.section).sort()).toEqual(['AB2R', 'AB3R']);
		const ab2r = groups.find((g) => g.section === 'AB2R');
		expect(ab2r?.sections.map((s) => `${s.section}/${s.type}`)).toEqual(['AB2/LEC', 'AB2R/RCT']);
	});
});
