import { fetchJsonWithRetry } from '$lib/utils/local/data/fetch-json';

export const ROOM_FIELD_CATEGORIES = ['directions', 'schedule', 'position'] as const;
...
export function buildRoomProgressRows(rooms: RoomProgressInput[]): RoomProgressRow[] {
	return rooms.map(buildRoomProgressRow);
}

export function buildingGapScore(counts: Record<RoomFieldCategory, ProgressCount>): number {
...
export function aggregateHistoryByEntityType(rows: Array<{ entityType: string; count: number }>): {
	totalEdits: number;
	byEntityType: Record<string, number>;
} {
...
export function missingFieldLabel(roomCode: string, category: RoomFieldCategory): string {
...
export async function fetchContributorProgress(scope: 'campus'): Promise<CampusContributorProgress>;
export async function fetchContributorProgress(
	scope: 'building',
	buildingId: number
): Promise<BuildingContributorProgress>;
export async function fetchContributorProgress(scope: 'mine'): Promise<MineContributorProgress>;
export async function fetchContributorProgress(
