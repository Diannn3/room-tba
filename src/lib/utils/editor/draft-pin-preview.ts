...
export function buildDraftPinPreview(input: SuggestAdditionPreviewInput): DraftPinPreview | null {
	switch (input.kind) {
		...
		case 'create_place': {
			...
			return label ? { kind: 'place', label, placeCategory: input.placeCategory } : null;
		}
...
export function draftPinRowLabel(preview: DraftPinPreview | null, hasCoords: boolean): string {
