...
export type EventRecurrence = 'none' | 'annual' | 'every_1st_sem' | 'every_2nd_sem';
...
export function sortStoredEvents<T extends { priority: number; occurrenceStartsAt: string }>(
	events: T[]
): T[] {
...
function campusFormat(value: string | Date, options: Intl.DateTimeFormatOptions): string {
...
export function formatCampusRange(startsAt: string | Date, endsAt: string | Date): string {
