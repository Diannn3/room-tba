 ...
 */
export function normalizeClassTime(value: string): { label: string; minutes: number } | null {
...
export function parseGenericClassesCsv(text: string): GenericClassEntry[] {
	...
	const headers = parseCsvRow(lines[headerLine]).map((h) => h.trim().toLowerCase());
	const missing = GENERIC_CLASS_REQUIRED_HEADERS.filter((h) => !headers.includes(h));
	if (missing.length > 0) {
		throw new Error(
...
export function parseGenericClassesFile(text: string, kind: 'csv' | 'json'): GenericClassEntry[] {
	return kind === 'json' ? parseGenericClassesJson(text) : parseGenericClassesCsv(text);
}

/** Field-level validation. Every issue names the row number and field. */
export function validateGenericClassEntries(entries: GenericClassEntry[]): RowIssue[] {
...
export function formatRowIssues(issues: RowIssue[]): string {
	return issues.map((issue) => `  row ${issue.row}: ${issue.field} ${issue.message}`).join('\n');
}
 ...
 */
export function normalizeGenericClasses(entries: GenericClassEntry[]): NormalizedGenericClass[] {
