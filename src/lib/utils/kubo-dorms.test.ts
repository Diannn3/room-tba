	...
	dorms: [
		{
			...
			reservationUrl: 'https://kubo.community/dorms/scholar-s-dormitory/apply/type',
...
describe('parseKuboDormDirectory', () => {
	test('accepts the versioned Kubo directory', () => {
		expect(parseKuboDormDirectory(acceptingDirectory)).toEqual(acceptingDirectory);
	});
...
describe('getKuboDormCta', () => {
	test('uses the direct reservation link only while accepting', () => {
		...
		const directory = new Map(parsed.dorms.map((record) => [record.roomTbaDormId, record]));
		...
		expect(getKuboDormCta(directory, 15, "Scholar's Dormitory")).toEqual({
			...
			ariaLabel: "Reserve a place at Scholar's Dormitory on Kubo (opens in new tab)"
		});
	});
	...
	test('uses the waitlist CTA and falls back to its listing URL', () => {
		...
		expect(
			getKuboDormCta(new Map([[record.roomTbaDormId, record]]), 15, "Scholar's Dormitory")
		).toEqual({
			href: record.listingUrl,
			label: 'Join waitlist on Kubo',
			ariaLabel: "Join the waitlist for Scholar's Dormitory on Kubo (opens in new tab)"
		});
	});
	...
	test('suppresses the CTA when the id-matched record names a different dorm', () => {
		...
		expect(getKuboDormCta(new Map([[1, record]]), 1, "Men's Residence Hall")).toBeNull();
	});
	...
	test.each([
		...
		expect(getKuboDormCta(new Map([[15, record]]), 15, dormName)).not.toBeNull();
	});
