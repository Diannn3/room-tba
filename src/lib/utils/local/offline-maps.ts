...
export function getCampusTileCoords(minZoom = MIN_ZOOM, maxZoom = MAX_ZOOM): TileCoord[] {
...
export async function getTileTemplate(): Promise<TileSource | null> {
	...
	try {
		const style = injectMaptilerKey(await (await fetch(CAMPUS_MAP_STYLE_URL)).json());
