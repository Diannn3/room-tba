...
function seedSyncKeys(store: Map<string, string>) {
	store.set(
		'sync-key',
		JSON.stringify(Object.fromEntries(SYNC_TABLE_NAMES.map((table) => [table, 'stale'])))
	);
}
...
function statusAfter(afterCalls: number, terminal: SyncStatus) {
	...
	return () => {
		calls += 1;
		return calls > afterCalls ? terminal : { allSynced: false, syncError: null };
	};
}
...
describe('resyncCampusData', () => {
	...
	it('invalidates every sync key, not a subset', async () => {
		await resyncCampusData(statusAfter(1, { allSynced: true, syncError: null }));
	...
	it('reports success once the sync store settles', async () => {
		const outcome = await resyncCampusData(statusAfter(1, { allSynced: true, syncError: null }), {
			pollMs: 1
		});
	...
	it('times out instead of hanging in a busy state forever', async () => {
		const outcome = await resyncCampusData(() => ({ allSynced: false, syncError: null }), {
			pollMs: 1,
			timeoutMs: 20
		});
