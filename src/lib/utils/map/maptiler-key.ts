 ...
 */
export const FALLBACK_MAP_STYLE_URL = 'https://tiles.openfreemap.org/styles/positron';
...
async function maptilerTilesReachable(style: unknown): Promise<boolean> {
	const url = (style as { sources?: Record<string, { url?: string }> })?.sources?.openmaptiles?.url;
	if (!url) return true;
...
export async function loadCampusMapStyle<T>(): Promise<T> {
	...
	if (!(await maptilerTilesReachable(style))) {
		console.warn('MapTiler rejected the configured key; falling back to a keyless basemap.');
		return loadFallbackMapStyle<T>();
	}
