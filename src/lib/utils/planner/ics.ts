import { parseDays, parseScheduleTime } from '$lib/utils/schedule-renderer.js';
...
export function buildPlanIcs(
	...
	const rangeStart =
		termStart ?? new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
	...
	for (const section of plan.sections) {
		for (const scheduleStr of section.schedule ?? []) {
			...
			if (!parsed) continue;
			const dayIndices = parseDays(parsed.days).filter((d) => d >= 0 && d < BYDAY.length);
			if (dayIndices.length === 0) continue;

			const summary = escapeIcsText(`${section.courseCode} ${section.type} ${section.section}`);
			// With a term end, one recurring VEVENT covers every day via BYDAY;
			// without it, emit one plain event per day so no day is lost.
			const eventDayGroups = termEnd ? [dayIndices] : dayIndices.map((d) => [d]);
