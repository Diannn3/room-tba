...
export function mergePlannerState(
	...
	for (const plan of local.plans) if (!byId.has(plan.id)) byId.set(plan.id, plan);
...
export function parsePlanner(raw: string | null): PlannerPersisted {
	...
	try {
		...
		const planIds = new Set(plans.map((p) => p.id));
		const activePlanIdByTerm: Record<string, string> = {};
		if (persisted.activePlanIdByTerm && typeof persisted.activePlanIdByTerm === 'object') {
			for (const [termId, planId] of Object.entries(persisted.activePlanIdByTerm)) {
