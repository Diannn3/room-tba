...
import { getPlannerBlockColor, parseDays, parseScheduleTime } from '$lib/utils/schedule-renderer';
...
export async function renderPlanToPng(
	...
	const dayCount = Math.max(5, (usedDays.length ? Math.max(...usedDays) : 4) + 1);

	let startHour = 7;
	let endHour = 19;
	if (blocks.length) {
		startHour = Math.min(7, Math.floor(Math.min(...blocks.map((b) => b.start)) / 60));
		endHour = Math.max(18, Math.ceil(Math.max(...blocks.map((b) => b.end)) / 60));
	}
	...
	for (let d = 0; d < dayCount; d++) {
		ctx.fillText(DAY_NAMES[d] ?? '', gridLeft + colW * d + colW / 2, gridTop + 8);
	}
