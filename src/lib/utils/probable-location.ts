...
export type ProbableLocationSource = 'department' | 'course-history' | 'dept-history';
...
export function findDeptEntity(orgs: OrgEntityRow[], wanted: string): OrgEntityRow | null {
...
export function rankBuildingsByHistory(
	...
	const termIds = [...new Set(rows.map((row) => row.termId))].sort((a, b) => a - b);
	const termWeight = new Map(termIds.map((id, index) => [id, index + 1]));
	const scores = new Map<number, { buildingName: string; score: number; latestTermId: number }>();
	...
	return best ? { buildingId: best.buildingId, buildingName: best.buildingName } : null;
}
