...
import { formatNominatimAddress, reverseGeocodeCacheKey } from './reverse-geocode';

describe('reverseGeocodeCacheKey', () => {
	test('rounds coordinates for stable cache keys', () => {
		expect(reverseGeocodeCacheKey(14.165738589, 121.240452825)).toBe('14.16574,121.24045');
	});
});
	...
	test('prefers structured address parts over the long display name', () => {
		expect(
			formatNominatimAddress({
				display_name: 'Carabao Park, Los Baños, Laguna, Calabarzon, 4030, Philippines',
				address: {
