 ...
 */
export function describeRoute(from: RoutePlace, to: RoutePlace, route: CampusRoute): string {
	const lines: string[] = [
	...
	for (const step of route.steps) {
		...
		lines.push(`After ${formatDistance(step.meters)}${where}, ${turnWord(step.modifier)}.`);
	}
