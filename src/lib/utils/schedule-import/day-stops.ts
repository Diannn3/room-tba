import { parseScheduleTime } from '$lib/utils/schedule-renderer';
...
export function scheduleSlotOnWeekday(scheduleStr: string, weekday: Weekday): boolean {
...
export function orderDayStops(matches: ScheduleMatchResult[], weekday: Weekday): ScheduleDayStop[] {
...
export function formatMinutes(minutes: number): string {
	...
	return min > 0 ? `${hour12}:${String(min).padStart(2, '0')} ${period}` : `${hour12} ${period}`;
}
