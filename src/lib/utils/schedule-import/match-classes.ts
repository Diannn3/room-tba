import { isRoomScheduledClassType, NON_ROOM_CLASS_TYPES } from '$lib/amis/room-scheduled-types';
import type { ClassMapValue } from '$lib/utils/types';
import type { ImportedScheduleRow, ScheduleMatchResult } from './types';

export function matchKey(courseCode: string, section: string, type: string): string {
	return `${courseCode.trim().toUpperCase()}::${section.trim().toUpperCase()}::${type.trim().toUpperCase()}`;
}

export type RoomCoordLookup = (roomCode: string) => [number, number] | null | undefined;
	...
	return rows.map((row) => {
		if (!isRoomScheduledClassType(row.type)) {
			...
			return {
				...
				coords: null,
				unresolvedReason:
					meta?.description ?? `${row.type} sections usually have no fixed room in Room TBA.`
			};
		}
