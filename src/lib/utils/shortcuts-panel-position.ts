...
export type ShortcutsPanelLayoutInput = {
	...
	triggerRect: DOMRectReadOnly | { left: number; top: number; right: number; bottom: number };
...
export function computeShortcutsPanelLayout(
	...
	const bottom = Math.max(margin, viewportHeight - triggerRect.top + triggerGap);
	...
	if (sidePanel) {
		...
		width = Math.min(maxPanelWidth, viewportWidth - margin * 2);
		left = Math.min(Math.max(margin, triggerRect.left), viewportWidth - width - margin);
	}
	...
	if (sidePanel) {
		...
		const mapWidth = Math.max(0, mapRight - mapLeft);
		if (mapWidth < 12 * 16) {
			const mapBandHeight = Math.max(0, triggerRect.top - triggerGap - sidePanel.top);
...
export function formatShortcutsPanelStyle(layout: ShortcutsPanelLayout): string {
...
export function measureOpenSidePanelBounds(): ShortcutsPanelBounds | null {
	...
	const handleRect = handle instanceof HTMLElement ? handle.getBoundingClientRect() : drawerRect;
