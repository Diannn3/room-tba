import { campusSite } from '$lib/utils/campus/campus.config';
...
export function jsonLd(...items: Array<Record<string, unknown> | null>) {
	// `filter(Boolean)` does not narrow, so the result stayed nullable and every
	// caller inherited it. Callers pass `cond ? {...} : null` freely, so the
	// predicate has to do the narrowing.
