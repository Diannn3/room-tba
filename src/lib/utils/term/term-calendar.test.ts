...
function term(id: number, overrides: Partial<Term> = {}): Term {
	...
	const semester = id === 1253 ? 'midyear' : id === 1252 ? '2' : String(id - 1250);
	return {
		id,
		label: `Term ${id}`,
...
describe('term-calendar', () => {
	it('detects midyear from Manila calendar date', () => {
		...
		expect(isDateWithinTerm(midyear, new Date('2026-06-29T12:00:00+08:00'))).toBe(true);
		expect(isDateWithinTerm(midyear, new Date('2026-08-01T12:00:00+08:00'))).toBe(false);
	});

	it('resolves the in-session term for today', () => {
		const terms = [term(1251), term(1252), term(1253)];
		expect(resolveActiveTermByDate(terms, new Date('2026-06-29T08:00:00+08:00'))?.id).toBe(1253);
		expect(resolveActiveTermByDate(terms, new Date('2026-03-15T08:00:00+08:00'))?.id).toBe(1252);
	});
...
describe('isFinalsWeek', () => {
	it('detects 2nd sem finals window in Manila', () => {
		...
		expect(isFinalsWeek(new Date('2026-05-18T12:00:00+08:00'), term(1252))).toBe(true);
		expect(isFinalsWeek(new Date('2026-05-14T12:00:00+08:00'), term(1252))).toBe(false);
	});
	...
	it('loads AY 2026-2027 windows from the academic calendar data', () => {
		...
		expect(isFinalsWeek(new Date('2026-12-03T12:00:00+08:00'), term(1261))).toBe(true);
		expect(isFinalsWeek(new Date('2026-12-08T12:00:00+08:00'), term(1261))).toBe(false);
	});
});
