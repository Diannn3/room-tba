...
export const PATCH: RequestHandler = async ({ cookies, params, request }) => {
	...
	if (body.category !== undefined && !ORG_CATEGORIES.includes(body.category as OrgCategory)) {
	...
	const parsedImageUrl = parseImageUrl(body.imageUrl, env.R2_PUBLIC_URL, 'Organization image');
	...
	try {
		...
		if (body.buildingId !== undefined) {
			updates.buildingId = body.buildingId === null ? null : Number(body.buildingId);
		}
		...
		if (body.lon !== undefined) updates.lon = body.lon === null ? null : Number(body.lon);
		if (body.description !== undefined) updates.description = body.description || null;
		if (body.websiteLink !== undefined) updates.websiteLink = body.websiteLink || null;
		if (body.facebookLink !== undefined) updates.facebookLink = body.facebookLink || null;
