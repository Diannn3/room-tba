...
export const PATCH: RequestHandler = async ({ cookies, params, request }) => {
	...
	if (
		(body.lat !== undefined && body.lat !== null && !Number.isFinite(Number(body.lat))) ||
		(body.lon !== undefined && body.lon !== null && !Number.isFinite(Number(body.lon)))
	) {
	...
	try {
		...
		const place = await updatePlace(id, updates, parsedVersion.version, auth.editedBy);
