...
const SIGNUP_RATE_LIMIT_MESSAGE = 'Too many sign-up attempts. Wait about a minute and try again.';
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		try {
			...
			return json(
				{
					error: 'Sign-up is not configured on this server. Contact the site maintainer.'
				},
				503
			);
		}
		...
		return json({ error: 'Sign-up is temporarily unavailable. Try again in a moment.' }, 503);
	}
};
