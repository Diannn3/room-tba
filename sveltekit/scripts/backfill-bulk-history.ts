...
import { flagValue, openDb, readOperations, reportResult } from './record-bulk-history';

/** Where the maintainer session left its pre-operation dumps. */
const DEFAULT_SNAPSHOT_ROOT = '/tmp/claude-1000/-home-stimmie-dev-uplbtools-room-tba';
...
function refCounts(snapshot: unknown): Record<string, number> {
	const refs = (snapshot as { refs?: Record<string, unknown[]> } | undefined)?.refs;
	if (!refs) return {};
	return Object.fromEntries(
		Object.entries(refs).map(([table, rows]) => [table, Array.isArray(rows) ? rows.length : 0])
	);
}
 ...
 */
export function buildBackfillOperations(snapshots: Snapshots = {}): BulkOperation[] {
	...
	if (mapRooms.length > 0) {
		operations.push({
			...
			reason: 'Map-position snapshot taken alongside the 2026-08-03 room corrections.'
		});
	}
...
async function main(): Promise<void> {
	...
	const operations = opsPath ? readOperations(opsPath) : buildBackfillOperations(snapshots);
