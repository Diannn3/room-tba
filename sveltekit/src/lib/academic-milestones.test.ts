...
describe('milestonesForTerms', () => {
	...
	const find = (label: string) => firstSem.find((milestone) => milestone.label === label);
	...
	it('keeps staff-only rows out of the UI but in the data', () => {
		...
		expect(labels.some((label) => label.includes('College Faculty'))).toBe(false);
	});
...
describe('holidaysWithin', () => {
	it("returns the official holidays inside the strip's span", () => {
		...
		expect(holidays.map((holiday) => holiday.label)).toContain('Ninoy Aquino Day');
