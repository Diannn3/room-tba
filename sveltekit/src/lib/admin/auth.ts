...
export function getSessionUser(cookieValue: string | undefined): SessionUser | null {
	...
	try {
		...
		return {
			...
			role:
				payload.role === 'admin' || payload.role === 'editor' || payload.role === 'contributor'
					? payload.role
					: 'contributor'
		};
	} catch {
...
export function parsePublishingActor(
	...
	return Number.isInteger(userId) && userId > 0 && displayName ? { userId, displayName } : null;
}
