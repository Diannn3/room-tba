...
describe('signed-token', () => {
	test('round-trips a valid token', () => {
		const token = createSignedToken<Payload>({ userId: 42, newEmail: 'a@example.com' }, 60, SECRET);
	...
	test('rejects an expired token', () => {
		const token = createSignedToken<Payload>({ userId: 1, newEmail: 'b@example.com' }, -1, SECRET);
		expect(verifySignedToken<Payload>(token, SECRET)).toBeNull();
	});

	test('rejects a tampered token', () => {
		const token = createSignedToken<Payload>({ userId: 1, newEmail: 'c@example.com' }, 60, SECRET);
	...
	test('rejects a token signed with a different secret', () => {
		const token = createSignedToken<Payload>({ userId: 1, newEmail: 'd@example.com' }, 60, SECRET);
