...
describe('isProposalHoneypotTripped', () => {
	test('empty honeypot is allowed', () => {
		expect(isProposalHoneypotTripped({ [PROPOSAL_HONEYPOT_FIELD]: '' })).toBe(false);
		expect(isProposalHoneypotTripped({})).toBe(false);
	});

	test('non-empty honeypot trips', () => {
		expect(isProposalHoneypotTripped({ [PROPOSAL_HONEYPOT_FIELD]: 'spam' })).toBe(true);
	});
});
...
describe('enforceProposalWithdrawLimits', () => {
	test('blocks after withdraw cap', () => {
		...
		for (let i = 0; i < 12; i += 1) {
			expect(enforceProposalWithdrawLimits({ id: 7 }, '203.0.113.1', now)).toBeNull();
		}
		const blocked = enforceProposalWithdrawLimits({ id: 7 }, '203.0.113.1', now);
		expect(blocked?.allowed).toBe(false);
	});
});
