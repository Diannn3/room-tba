...
describe('findCampusPointBySlug', () => {
	...
	test('returns [lon, lat], the order MapLibre wants', () => {
		...
		expect(findCampusPointBySlug(data, 'cem-building')).toEqual([121.24, 14.16]);
	});

	test('resolves across buildings, dorms and places', () => {
		expect(findCampusPointBySlug(data, 'men-s-residence-hall')).toEqual([121.25, 14.17]);
		expect(findCampusPointBySlug(data, 'carabao-park')).toEqual([121.253, 14.166]);
	});

	test('skips entries with no coordinates instead of returning nulls', () => {
		expect(
			findCampusPointBySlug({ places: [{ name: 'Unplaced', lat: null, lon: null }] }, 'unplaced')
		).toBeNull();
	});
...
describe('generateRouteDescription', () => {
	test('returns null without an API key and never calls out', async () => {
		...
		expect(await generateRouteDescription(baker, carabao, route, undefined, spy)).toBeNull();
	...
	test('falls back to null when the API errors, so the page still renders', async () => {
		...
		expect(await generateRouteDescription(baker, carabao, route, 'key', failing)).toBeNull();
	});
	...
	test('returns the model text when the call succeeds', async () => {
		...
		expect(await generateRouteDescription(baker, carabao, route, 'key', ok)).toBe(
			'Head out of Baker Hall.'
		);
	});
});
...
describe('sumRouteLegs', () => {
	...
	test('skips legs the OSRM response left non-numeric', () => {
		expect(
			sumRouteLegs([{ distance: 100, duration: 80 }, { distance: undefined, duration: 80 }, {}])
