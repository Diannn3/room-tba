...
export const OSRM_FOOT_API = 'https://routing.openstreetmap.de/routed-foot/route/v1';
...
export function toCampusRoute(payload: OsrmResponse, landmarks: RoutePlace[]): CampusRoute | null {
	...
	for (const leg of route.legs ?? []) {
		for (const step of leg.steps ?? []) {
			...
			steps.push({
				...
				landmark: location
					? (nearestPlace({ lon: location[0], lat: location[1] }, landmarks)?.name ?? null)
					: null
			});
		}
	}
...
export function sumRouteLegs(
	...
	return counted ? { meters: Math.round(meters), seconds: Math.round(seconds) } : null;
}
