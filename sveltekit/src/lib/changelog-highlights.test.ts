...
`;
		...
		expect(result?.items).toEqual(['feat: campus events on the map', 'fix: mobile search chrome']);
...
`;
		...
		expect(result?.items).toEqual(['account: include saved planner plans in the data export']);
	});
});
...
`;
		...
		expect(entries[0]).toEqual({
			version: '1.36.0',
			date: '2026-07-09',
			sections: [{ title: 'Features', items: ['account: include saved planner plans'] }]
		});
