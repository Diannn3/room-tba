<script lang="ts">
	...
	import Modal from '$lib/components/svelte/modal/Modal.svelte';
	import MainControls from '$lib/components/svelte/controls/MainControls.svelte';
	import Map from '$lib/components/svelte/Map.svelte';
	import MapAttribution from '$lib/components/svelte/MapAttribution.svelte';
	import MeasureRoutePanel from '$lib/components/svelte/MeasureRoutePanel.svelte';
	import TravelTimeLegend from '$lib/components/svelte/TravelTimeLegend.svelte';
	import Toast from '$lib/components/svelte/Toast.svelte';
	import Building3DViewer from '$lib/components/svelte/Building3DViewer.svelte';
	import AdminLoginModal from '$lib/components/svelte/AdminLoginModal.svelte';
	import AccountSettingsModal from '$lib/components/svelte/AccountSettingsModal.svelte';
	import ManageUsersModal from '$lib/components/svelte/modal/ManageUsersModal.svelte';
	import EditorAdditionModal from '$lib/components/svelte/EditorAdditionModal.svelte';
	import PlannerScreen from '$lib/components/svelte/planner/PlannerScreen.svelte';
	import FinalExamsScreen from '$lib/components/svelte/final-exams/FinalExamsScreen.svelte';
	...
	import EntityUrlSync from '$lib/components/svelte/EntityUrlSync.svelte';
	import EntityHoverPreview from '$lib/components/svelte/map/EntityHoverPreview.svelte';
		...
		{#if ['map', 'contributors', 'settings'].includes(sidebarStore.panelOpen)}
			{#if mobile.current}
				<div
					...
					class:mobile-map-controls--sheet-open={sidePanelStore.mobileSheetSnap !== 'closed'}
...
<style>
	.app-layout {
       ...
       edge on production, where there is no banner to clear. */
		...
		--side-panel-bottom-inset: calc(
			var(--side-panel-bottom-inset-measured) + var(--side-panel-bottom-gap, 0.375rem)
		);
		...
		--mobile-detail-sheet-top-inset: calc(
			var(--staging-banner-height, 0px) + var(--staging-banner-gap, 0.5rem) +
				var(--search-block-height) + var(--map-tools-block-height) + var(--map-ui-padding) * 2 +
	...
	.app-layout.edit-mode {
		...
		--side-panel-bottom-inset: calc(
			var(--side-panel-bottom-inset-measured) + var(--side-panel-bottom-gap, 0.375rem) +
				var(--edit-bar-height, 0rem) + var(--bottom-fab-gap, 0.5rem)
		);
	}
	...
	.bottom-band::before {
		...
		bottom: calc(-1 * (var(--map-ui-padding, 0.5rem) + env(safe-area-inset-bottom, 0px)));
		height: calc(6rem + var(--map-ui-padding, 0.5rem) + env(safe-area-inset-bottom, 0px));
	...
	.ui-layer {
       ...
       Do not use `inset: 0` here — it resets `top` back to 0. */
		position: fixed;
		top: calc(var(--staging-banner-height, 0px) + var(--staging-banner-gap, 0.5rem));
		right: 0;
		bottom: 0;
		left: 0;
		z-index: 10;
	...
	.app-layout.redesign-desktop :global(.drawer-card .entity-panel-close:focus-visible) {
		background: #fff;
	...
	.app-layout.redesign-desktop :global(.drawer-card .entity-panel-filter--search) {
	...
	.app-layout.redesign-desktop :global(.drawer-card .entity-panel-filter--search:focus-within) {
		outline: none;
		border-color: transparent;
		box-shadow:
			var(--shadow-search, 0 1px 3.5px rgb(58 58 71 / 0.2)),
	...
	.app-layout.redesign-desktop :global(.drawer-card .entity-panel-filter--search input) {
		padding: 0.35rem 0;
	}

	.app-layout.redesign-desktop :global(.drawer-card .map-chrome-action-chip),
	.app-layout.redesign-desktop :global(.drawer-card .map-chrome-action-chip--toolbar),
	...
	.app-layout.redesign-desktop :global(.drawer-card .map-chrome-action-chip:hover:not(:disabled)),
	.app-layout.redesign-desktop
		:global(.drawer-card .map-chrome-action-chip--toolbar:hover:not(:disabled)),
	.app-layout.redesign-desktop :global(.drawer-card .editor-toggle--toolbar:hover) {
		border-color: transparent;
		background: #fff;
		box-shadow:
			var(--shadow-search, 0 1px 3.5px rgb(58 58 71 / 0.2)),
	...
	@media (max-width: 48rem) {
		...
		.ui-layer > :global(.acal-screen) {
			padding-bottom: calc(
				1rem + var(--mobile-bottom-nav-height, 4.5rem) + env(safe-area-inset-bottom, 0px)
			);
			box-sizing: border-box;
		}
	}
	...
	@media (max-width: 48rem) {
		...
		.top-right-map-stack :global(.map-chrome-panel) {
         ...
         search chrome, instead of butting edge-to-edge against the top bar. */
			...
			max-width: none;
			max-height: calc(
				100dvh - var(--staging-banner-height, 0px) - var(--staging-banner-gap, 0.5rem) -
					var(--search-block-height) - var(--map-ui-padding, 0.375rem) -
