...
<div class="map-shell">
			...
			>
				...
				{#if measureRouteStore.active}
					{#each measureRouteStore.waypoints as waypoint, i (i)}
						{@const snapped =
							measureRouteStore.snapped.length === measureRouteStore.waypoints.length
				...
				{#if additionProposalStore.draftPin}
					...
					>
						{#if additionProposalStore.draftPinPreview}
							<ContributorDraftPinMarker preview={additionProposalStore.draftPinPreview} />
						{:else}
					...
					{#if editableEventLocation}
						...
						>
							<div class="event-marker-anchor event-edit-anchor">
								<span class="event-anchor-dot event-edit-dot" aria-hidden="true"></span>
								<div
									...
									{#if group.entries.length === 1}
										...
										{#if entry}
											...
											{@const previewSuppressed =
												centralHoverPreview &&
												isEventHoverPreview(entityHoverPreviewStore.entity, entry.event.slug)}
											<EventMapPin
												{active}
												...
												dateLabel={formatEventMarkerDate(entry.event.occurrenceStartsAt)}
												...
												onpointerenter={(event) => handleEventPinPointerEnter(entry.event, event)}
									...
									{:else}
										...
										{#if primaryEntry}
											...
											<EventMapPin
												...
												dateLabel={formatEventMarkerDate(primaryEntry.event.occurrenceStartsAt)}
														...
														{#each group.entries as entry (`event-stack:${entry.event.id}:${entry.location.id}`)}
															...
															>
																...
																<span class="event-stack-copy">
																	<span class="event-stack-title">{entry.event.title}</span>
																	<span class="event-stack-meta">
																		{formatEventMarkerDate(entry.event.occurrenceStartsAt)}
																		at {formatEventMarkerTime(entry.event.occurrenceStartsAt)}
						...
						{#if place.lat != null && place.lon != null && (poiPinsVisible || sponsoredPlacePins.has(place.name) || (queryStore.category === 'place' && queryStore.inputValue === place.name))}
							...
							<Marker lngLat={[place.lon, place.lat]}>
								<MapEntityPin
									...
									onpointerenter={(event) => handlePlacePinPointerEnter(place, event)}
		...
		{#if md.current}
			...
			>
				<p class="edit-dock-status">
					{eventPlacementStore.creating ? 'Creating event…' : 'Tap the map to place the event'}
				</p>
				<button
	...
	{:else if additionProposalStore.pinPickActive}
		{#if md.current}
			...
			>
				<p class="edit-dock-status">
					Tap the map to set the pin location. The preview shows how it will look after approval.
				</p>
				<button
					class="edit-dock-action cancel"
					type="button"
		...
		{:else}
			...
			>
				<div class="event-placement-copy">
					...
					<span>
						Click or tap the map where this entry should appear. The preview pin shows how it will
						look after approval.
					</span>
				</div>
				<button
	...
	{:else if isMapPinSuggestMode()}
		<div bind:this={editChromeEl} class="edit-dock map-edit-dock" role="status" aria-live="polite">
			<p class="edit-dock-status">Drag the highlighted pin, then release to submit for review.</p>
		</div>
	{:else if isMapEditEnabled()}
			...
			>
				...
				<div class="edit-dock-actions">
					<button
						...
						title={undoMove ? `Undo move for ${undoMove.name}` : 'No move to undo yet'}
					...
					<button
						...
						title={redoMove ? `Redo move for ${redoMove.name}` : 'No move to redo yet'}
		...
		{:else}
			<div bind:this={editChromeEl} class="map-edit-toolbar" role="status" aria-live="polite">
...
<style>
	...
	.map-edit-toolbar {
		...
		bottom: calc(
			var(--status-bar-block-height, 2.75rem) +
				var(--bottom-fab-gap, var(--map-ui-padding, 0.5rem)) + env(safe-area-inset-bottom, 0px)
		);
		left: calc(
	...
	.edit-dock {
		...
		right: calc(var(--bottom-fab-inset, 3.25rem) + max(0.5rem, env(safe-area-inset-right, 0px)));
		bottom: calc(
			var(--status-bar-block-height, 2.75rem) +
				var(--bottom-fab-gap, var(--map-ui-padding, 0.5rem)) + env(safe-area-inset-bottom, 0px)
		);
