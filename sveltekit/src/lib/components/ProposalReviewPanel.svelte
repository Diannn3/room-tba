<script lang="ts">
	...
	const NOTE_HINTS: Record<NoteAction, string> = {
		...
		'request-changes': 'Required. Say what needs to change so the contributor can revise.'
	};
	...
	function isStale(proposal: { currentVersion?: number | null; baseVersion: number }) {
		return proposal.currentVersion != null && proposal.currentVersion !== proposal.baseVersion;
	}
	...
	async function approveOne(id: number): Promise<{ ok: boolean; error?: string }> {
		...
		const entityType = (data as { proposal?: { entityType?: string } }).proposal?.entityType;
		if (entityType) {
			afterProposalPublished(appActions, appData, entityType as ProposalEntityType, published);
			syncOpenEntityQueryAfterPublish(appData, entityType as ProposalEntityType, published);
		}
		noteById[id] = '';
		return { ok: true };
	}
	...
	async function commitHeldApproval() {
		...
		try {
			...
			if (!result.ok) {
				toastStore.show(result.error ?? `Could not approve proposal #${held.id}.`, 'error');
			} else {
	...
	async function runAction(id: number, action: ReviewAction) {
		if (action === 'approve') {
			const label = proposalsStore.proposals.find((p) => p.id === id)?.entityLabel ?? `#${id}`;
		...
		try {
			...
			if (!result.ok) {
				toastStore.show(result.error ?? `Could not ${action} proposal #${id}.`, 'error');
				return;
			}
			selectedIds.delete(id);
			pendingActionById[id] = null;
			toastStore.show(
				action === 'reject' ? 'Proposal rejected.' : 'Sent back to contributor with notes.',
				'success'
			);
			noteById[id] = '';
			await proposalsStore.refresh();
	...
	function handleShortcut(event: KeyboardEvent) {
		...
		if (action === 'next' || action === 'previous') {
			event.preventDefault();
			focusRow(stepIndex(focusedIndex, flatProposals.length, action === 'next' ? 1 : -1));
			return;
		}
	...
	async function runBatch(action: ReviewAction) {
		...
		try {
			for (const id of ids) {
				const result =
					action === 'approve' ? await approveOne(id) : await reviewOne(id, action, bulkNote);
				if (result.ok) {
					done += 1;
					selectedIds.delete(id);
				} else {
...
{#snippet diffValue(text: string | null, id: number | undefined)}
	{text ?? '—'}{#if id != null}<small class="entity-review-diff-id">#{id}</small>{/if}
{/snippet}
	...
	<section class="entity-review-panel" aria-label="Edit proposals review queue">
		<div class="entity-review-heading">
			...
			{#if proposalsStore.pendingCount > 0}
				<span class="entity-review-count">{proposalsStore.pendingCount} pending</span>
			{/if}
		</div>
		...
		{:else}
			<div class="entity-review-batch">
				...
				<div class="entity-review-batch-actions">
					...
					>
						{batchRunning && bulkAction === null ? 'Approving…' : 'Approve selected'}
					</button>
					<button
						type="button"
						disabled={selectedIds.size === 0 || batchRunning}
			...
			{#if bulkAction}
				<div class="entity-review-bulk-note">
					...
					>
						{#snippet control()}
							<textarea
								...
								rows="2"
								bind:value={bulkNote}
								aria-describedby="proposal-bulk-note-hint"></textarea>
						{/snippet}
					</EntityEditorFormField>
					<div class="entity-review-actions">
						...
						>
							{batchRunning ? 'Working…' : `${ACTION_LABELS[bulkAction]} ${selectedIds.size}`}
						</button>
						<button
							type="button"
							disabled={batchRunning}
					...
					<li class="entity-review-group">
						<div class="entity-review-group-head">
							...
							<span class="entity-review-count">
								{group.proposals.length} suggestion{group.proposals.length === 1 ? '' : 's'}
							</span>
						</div>
						...
						<ul class="entity-review-list entity-review-sublist">
							{#each group.proposals as proposal (proposal.id)}
								...
								<li
									...
									class:entity-review-item--focused={focusedProposal?.id === proposal.id}
								...
								>
										...
										<details bind:open={openById[proposal.id]}>
											<summary>
												...
												<span class="entity-review-summary-meta">
													{proposal.entityType} · {diffs.length} field{diffs.length === 1
														? ''
														: 's'}
											...
											{#if openById[proposal.id]}
												{#if stale}
													<p class="entity-review-stale" role="alert">
														Published data changed since this was submitted. Compare carefully —
														approving may conflict.
													</p>
												{/if}
															...
															<span class="entity-review-diff-values">
																...
																>
																	{#if proposal.currentValues == null && proposal.entityType.startsWith('create_')}
																		New entry
																	{:else}
																		{@render diffValue(diff.before, diff.beforeId)}
																	{/if}
																</span>
												...
												{#if pinChange}
													<div class="entity-review-preview">
														<button
															...
															aria-expanded={previewOpenById[proposal.id] === true}
															onclick={() =>
																(previewOpenById[proposal.id] = !previewOpenById[proposal.id])}
														>
                                   ...
                                   ask for a preview. -->
															...
															{:then previewModule}
																...
																<Preview change={pinChange} label={proposal.entityLabel} />
															{:catch}
												...
												{#if pendingActionById[proposal.id]}
													{@const action = pendingActionById[proposal.id] as NoteAction}
													<EntityEditorFormField
													...
													>
														{#snippet control()}
															<textarea
																...
																aria-describedby="proposal-note-{proposal.id}-hint"></textarea>
														{/snippet}
													</EntityEditorFormField>
													<div class="entity-review-actions">
														<button
														...
														<button
															type="button"
															disabled={actingId === proposal.id}
															onclick={() => (pendingActionById[proposal.id] = null)}
														>
															Cancel
														</button>
													</div>
												{:else}
													<EntityReviewActions
														...
														onrequestChanges={() =>
															(pendingActionById[proposal.id] = 'request-changes')}
														onreject={() => (pendingActionById[proposal.id] = 'reject')}
													/>
												{/if}
											{/if}
										</details>
