<script lang="ts">
	...
	const collegeShareUrl = $derived(college ? getCollegeShareUrl(college.collegeName) : '');
	...
	$effect(() => {
		...
		void (async () => {
			const counts = await fetchRoomClassCounts('college', id, termId ?? undefined);
	...
	async function saveName() {
		...
		try {
			...
			if (outcome.proposal) {
				...
				toastStore.show('College name suggestion submitted for review.', 'success');
			}
	...
	{#if college}
		<header class="entity-header">
			...
			<div class="entity-actions">
				<EntityShareCopyLink url={collegeShareUrl} entityLabel={college.collegeName} />
				<EntityEditorToggle
					expanded={editing}
					{canPublish}
		...
		<EntityLastUpdated updatedAt={college.updatedAt} entityType="college" entityId={college.id} />
		...
		<section class="entity-list-section" aria-label="College divisions">
			...
			{:else}
				<p class="entity-nav-empty">
					No divisions assigned yet. Open a division and set its parent college in the editor.
				</p>
			{/if}
		</section>
	{/if}
