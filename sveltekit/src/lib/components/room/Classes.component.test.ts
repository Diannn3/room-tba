...
describe('Classes', () => {
	...
	test('renders the probable-location hint for a Room TBA section (#846)', () => {
		...
		expect(screen.getByText('Offered by Institute of Computer Science (CAS)')).toBeVisible();
		expect(
	...
	test('hedges with the modal building for history-sourced hints (#846)', () => {
		...
		expect(screen.getByText('Usually meets around Physical Sciences Building')).toBeVisible();
	...
	test('shows no hint or pin for assigned sections and hint-less rows', () => {
		mountAtWidth(320);
		render(Classes, {
			classes: [{ ...tbaLecture, id: 3, roomId: 9, roomCode: 'ICS MH3' }, roomlessClass]
		});

		expect(screen.queryByText(/Usually meets around|Offered by/)).toBeNull();
		expect(screen.queryByRole('button', { name: /Show probable location/ })).toBeNull();
	});
});
