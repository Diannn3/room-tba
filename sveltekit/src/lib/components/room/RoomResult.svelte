	...
	{#if currentRoom.value}
		<header class="entity-header">
			...
			{#if roomCategoryLabel(currentRoom.value.category)}
				<span class="room-category-badge">{roomCategoryLabel(currentRoom.value.category)}</span>
			{/if}
			...
			<div class="entity-actions">
				...
				{/if}
				<EntityShareCopyLink url={roomShareUrl} entityLabel={currentRoom.value.code} />
				<EntityEditorToggle
					expanded={editing}
					{canPublish}
			...
			<section class="entity-editor" aria-label="Edit room details">
				...
				>
					...
					<EntityEditorField
						...
						stacked
						unchanged={directionsDraft.trim() === (currentRoom.value.directions ?? '')}
					...
					<div class="editor-grid">
						<EntityEditorField
							...
							unchanged={buildingDraft === String(currentRoom.value.buildingId ?? '')}
						...
						<EntityEditorField
							...
							unchanged={collegeDraft === String(currentRoom.value.collegeId ?? '')}
						...
						<EntityEditorField
							...
							unchanged={divisionDraft === String(currentRoom.value.divisionId ?? '')}
							...
							{#snippet control()}
								...
								>
									...
									{#each divisions as division (division.id)}
										<option value={String(division.id)}>{division.divisionName}</option>
									{/each}
								</select>
							{/snippet}
						</EntityEditorField>
					...
					{#if mergePrompt}
						<div class="merge-prompt" role="status">
							<p>
								...
								<strong>{currentRoom.value.code}</strong> into it? Classes and map pins move to the kept
								room; empty fields are filled from the merged room.
							</p>
		...
		<section class="entity-list-section entity-schedule" aria-label="Classes in this room">
			<div class="entity-schedule__header">
				<h3 class="entity-section-heading">
					Classes in this room
					{#if !roomClassesStore.loading}
						<span class="entity-schedule__count">({roomClassesStore.classes.length})</span>
					{/if}
				</h3>
			...
			{:else}
				<p class="entity-schedule__empty">
					No classes listed for this room{activeTermLabel ? ` in ${activeTermLabel}` : ''}.
				</p>
			{/if}
		</section>
