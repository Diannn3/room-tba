...
describe('buildFieldDiffs', () => {
	...
	test('treats empty strings as unset', () => {
		const diffs = buildFieldDiffs({ description: '' }, { description: 'Now filled' });
...
describe('buildFieldDiffs foreign key resolution (#873)', () => {
	const resolveName = (field: string, id: number) => {
		if (field === 'buildingId') {
			return ({ 3: 'Physical Sciences', 24: 'CHE Building' } as Record<number, string>)[id] ?? null;
		}
		if (field === 'collegeId') {
	...
	test('renders entity names and keeps the ids as secondary values', () => {
		const diffs = buildFieldDiffs({ buildingId: 3 }, { buildingId: 24 }, resolveName);
		expect(diffs).toEqual([
			{
				field: 'buildingId',
				label: 'Building',
	...
	test('falls back to the raw id when the resolver has no name', () => {
		const diffs = buildFieldDiffs({ buildingId: 3 }, { buildingId: 99 }, resolveName);
	...
	test('still detects a change when two ids share a display name', () => {
		...
		const diffs = buildFieldDiffs({ buildingId: 3 }, { buildingId: 24 }, sameName);
	...
	test('leaves non-id fields untouched', () => {
		const diffs = buildFieldDiffs({ capacity: 10 }, { capacity: 12 }, resolveName);
		expect(diffs[0]).toEqual({
			field: 'capacity',
			label: 'Capacity',
			before: '10',
