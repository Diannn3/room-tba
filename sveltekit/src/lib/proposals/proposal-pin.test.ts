...
import { pinChangeBounds, pinMoveDistanceMeters, proposalPinChange } from './proposal-pin';
...
describe('proposalPinChange', () => {
	...
	test('no preview when the patch does not move a pin', () => {
		expect(proposalPinChange({ proposedPatch: { directions: 'Beside the oval' } })).toBeNull();
	});
	...
	test('a lone coordinate is not a location', () => {
		...
		expect(proposalPinChange({ proposedPatch: { lat: 'nope', lon: 121.24 } })).toBeNull();
	});

	test('accepts numeric strings, which is how form patches arrive', () => {
		expect(
			proposalPinChange({ proposedPatch: { lat: '14.1663', lon: '121.2412' } })?.after
...
describe('pinChangeBounds', () => {
	...
	test('returns null when there is only the proposed pin', () => {
		expect(pinChangeBounds({ before: null, after: { lat: 14.166, lon: 121.241 } })).toBeNull();
	});
});
