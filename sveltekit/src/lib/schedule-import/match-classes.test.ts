...
describe('matchImportedScheduleRows', () => {
	...
	it('matches course section type to institutional class', () => {
		const [result] = matchImportedScheduleRows([row], [sampleClass], () => [121.077, 14.135]);
	...
	it('skips thesis sections for routing', () => {
		const [result] = matchImportedScheduleRows([{ ...row, type: 'THE' }], [sampleClass], () => [
			121, 14
		]);
	...
	it('flags rooms without coordinates', () => {
		const [result] = matchImportedScheduleRows([row], [sampleClass], () => null);
