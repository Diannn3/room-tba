...
export async function mergeRooms({
	...
	const [source, target] = await Promise.all([getRoomById(sourceId), getRoomById(targetId)]);
	...
	if (normalizedMerged !== sourceNormalized && normalizedMerged !== targetNormalized) {
	...
	await db.transaction(async (tx) => {
		const [lockedSource] = await tx
			...
			.from(roomsTable)
			.where(and(eq(roomsTable.id, sourceId), eq(roomsTable.version, sourceVersion)))
			.limit(1);
		...
		if (sourcePosition && !targetPosition) {
			...
			await tx.delete(roomPositionsTable).where(eq(roomPositionsTable.roomId, sourceId));
		}
...
export async function mergeBuildings({
	...
	const mergedName = pickMergedLabel(target.buildingName, source.buildingName, preferredName);
	...
	await db.transaction(async (tx) => {
		const [lockedSource] = await tx
			...
			.from(buildingsTable)
			.where(and(eq(buildingsTable.id, sourceId), eq(buildingsTable.version, sourceVersion)))
			.limit(1);
		...
		await tx
			...
			.set({
				...
				directions: target.directions.trim() !== '' ? target.directions : source.directions,
...
export async function mergeColleges({
	...
	const [source, target] = await Promise.all([getCollegeById(sourceId), getCollegeById(targetId)]);

	if (!source) throw new Error(`College ${sourceId} was not found.`);
	if (!target) throw new Error(`College ${targetId} was not found.`);

	const mergedName = pickMergedLabel(target.collegeName, source.collegeName, preferredName);
	...
	await db.transaction(async (tx) => {
		const [lockedSource] = await tx
			...
			.from(collegesTable)
			.where(and(eq(collegesTable.id, sourceId), eq(collegesTable.version, sourceVersion)))
			.limit(1);
...
export async function mergeDivisions({
	...
	const mergedName = pickMergedLabel(target.divisionName, source.divisionName, preferredName);
	...
	await db.transaction(async (tx) => {
		const [lockedSource] = await tx
			...
			.from(divisionsTable)
			.where(and(eq(divisionsTable.id, sourceId), eq(divisionsTable.version, sourceVersion)))
			.limit(1);
...
export async function mergeDorms({
	...
	const [source, target] = await Promise.all([getDormById(sourceId), getDormById(targetId)]);

	if (!source) throw new Error(`Dorm ${sourceId} was not found.`);
	if (!target) throw new Error(`Dorm ${targetId} was not found.`);

	const mergedName = pickMergedLabel(target.dormName, source.dormName, preferredName);
	...
	await db.transaction(async (tx) => {
		const [lockedSource] = await tx
			...
			.from(dormsTable)
			.where(and(eq(dormsTable.id, sourceId), eq(dormsTable.version, sourceVersion)))
			.limit(1);
		...
		await tx
			...
			.set({
				dormName: mergedName,
				shortName: target.shortName?.trim() ? target.shortName : source.shortName,
				lat: target.lat ?? source.lat,
				lon: target.lon ?? source.lon,
				gender: target.gender?.trim() ? target.gender : source.gender,
				capacity: target.capacity ?? source.capacity,
				managingOffice: pickNullableText(target.managingOffice, source.managingOffice),
				contactEmail: pickNullableText(target.contactEmail, source.contactEmail),
				amenities:
					target.amenities && target.amenities.length > 0 ? target.amenities : source.amenities,
