...
export type SponsorTier = 'gold' | 'silver' | 'bronze' | 'promo' | 'student-org';
...
export function getActiveSponsors(sponsors: Sponsor[], now: Date = new Date()): Sponsor[] {
	return sponsors.filter(
		(s) => s.active && new Date(s.startDate) <= now && new Date(s.endDate) >= now
	);
}

export function getByTier(sponsors: Sponsor[], tier: SponsorTier, now?: Date): Sponsor[] {
	return getActiveSponsors(sponsors, now).filter((s) => s.tier === tier);
}

export function getGoldSponsor(sponsors: Sponsor[], now?: Date): Sponsor | null {
...
export function rotateSponsor(sponsors: Sponsor[], now: Date = new Date()): Sponsor | null {
 ...
 */
export function getSponsoredPlacePins(sponsors: Sponsor[], now?: Date): Map<string, string> {
	const pool = [...getByTier(sponsors, 'gold', now), ...getByTier(sponsors, 'silver', now)];
