...
export type WalkGraphEdge = [number, number, number, string, number | null, number[], number?];
...
export function buildTravelGraph(data: WalkGraphData): TravelGraph {
	...
	const adjacency: TravelGraph['adjacency'] = Array.from({ length: n }, () => []);
 ...
 */
export function edgeSeconds(edge: WalkGraphEdge, mode: TravelMode): number | null {
	const meters = edge[2];
...
export function nearestNodeIndex(graph: TravelGraph, lat: number, lng: number): number {
	let best = 0;
	let bestMeters = Number.POSITIVE_INFINITY;
	for (let i = 0; i < graph.lat.length; i++) {
		const meters = distanceMeters({ lat, lon: lng }, { lat: graph.lat[i], lon: graph.lng[i] });
	...
	const pop = () => {
		...
		if (heapTime.length > 0) {
			...
			for (;;) {
				...
				const right = left + 1;
				let smallest = i;
				if (left < heapTime.length && heapTime[left] < heapTime[smallest]) smallest = left;
				if (right < heapTime.length && heapTime[right] < heapTime[smallest]) smallest = right;
				if (smallest === i) break;
...
export function edgeCoordinates(graph: TravelGraph, edgeIndex: number): [number, number][] {
...
export function shortestPath(
	...
	segments.reverse();
	const coordinates = segments.flatMap((coords, i) => (i === 0 ? coords : coords.slice(1)));
