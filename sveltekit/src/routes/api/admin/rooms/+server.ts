...
export const POST: RequestHandler = async ({ cookies, request }) => {
	...
	const buildingId =
		body.buildingId === null || body.buildingId === undefined ? null : Number(body.buildingId);
	const collegeId =
		body.collegeId === null || body.collegeId === undefined ? null : Number(body.collegeId);
	const divisionId =
		body.divisionId === null || body.divisionId === undefined ? null : Number(body.divisionId);
	...
	if (
		...
		return json({ error: 'Invalid building, college, or division selection' }, 400);
	}

	const directions = typeof body.directions === 'string' ? body.directions.trim() || null : null;
