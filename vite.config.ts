	...
	plugins: [
		...
		SvelteKitPWA({
			...
			workbox: {
				...
				// globDirectory/swDest are left to @vite-pwa/sveltekit: it points them
				// at .svelte-kit/output (the parent of client/ and prerendered/) and
				// emits the worker into the client build. Patterns are relative to
				// that root, hence the client/ and prerendered/ prefixes.
				//
				// Precache app assets plus the prerendered shells so the app loads
				// offline. The navigate fallback serves the root shell (`/`, which the
				// plugin derives from prerendered/pages/index.html) for offline
				// navigations. Entity SEO pages are deliberately not prerendered, so
				// they cannot bloat this precache.
				globPatterns: [
					'client/**/*.{js,css,ico,png,svg,webmanifest,json,jpg}',
					'prerendered/**/*.html'
				],
				...
				runtimeCaching: [
					...
					},
					{
						...
						urlPattern: /\/_app\/immutable\/assets\/(pglite|initdb)[.-][^/?]*\.(wasm|data)(\?|$)/,
						handler: 'CacheFirst',
						options: {
					...
					},
					{
						...
						urlPattern: /^https:\/\/(maputnik|orangemug|klokantech)\.github\.io\/.*/i,
						handler: 'CacheFirst',
						options: {
